...
def rotate_dimer(geom0, geom1, geom2, N, R, f_thresh=2.5e-3, max_cycles=10, alpha=0.05):
    ...
    for i in range(max_cycles):
        ...
        coords.append((geom1.coords.copy(), geom0.coords.copy(), geom2.coords.copy()))
...
def dimer_method(
    ...
    trans_optimizer = lbfgs_closure(
        lambda _, *args: get_f_trans(*args), restrict_step=restrict_step
    )
    for i in range(max_cycles):
        f0 = geom0.forces
        f0_rms = rms(f0)

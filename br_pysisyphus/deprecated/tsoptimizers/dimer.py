...
DimerCycle = namedtuple(
    "DimerCycle",
    "org_coords rot_coords trans_coords f0 f_tran",
)

DimerPickle = namedtuple("DimerPickle", "coords0 N dR")

DimerResult = namedtuple("DimerResult", "dimer_cycles force_evals geom0 converged")
...
def dimer_method(
    ...
    print(
        f"Keeping information of last {trans_memory} cycles for "
        "translation optimization."
    )

    f_tran_dict = {True: get_f_tran_mod, False: get_f_tran_org}
    ...
    for i in range(max_cycles):
        ...
        trans_table.print(
            f"@ {i:03d}: C={C:.6f}; max(|f0|)={f0_max:.6f}; rms(f0)={f0_rms:.6f}"
        )
        print()
        ...
        rot_cbm = closures.modified_broyden_closure(
            cbm_rot_force_getter, **rot_cbm_kwargs
        )
        for j in range(max_rots):
            ...
            if rot_type == "fourier":
                ...
                if np.abs(rad_trial) < rad_tol:
                    logger.debug(
                        f"rad_trial={rad_trial:.2f} below threshold. Breaking."
                    )
                    break
                ...
                b1 = 0.5 * dC
                a1 = (C - C_trial + b1 * np.sin(2 * rad_trial)) / (
                    1 - np.cos(2 * rad_trial)
                )
                a0 = 2 * (C - a1)
                ...
                def get_C(theta_rad):
                    return (
                        a0 / 2 + a1 * np.cos(2 * theta_rad) + b1 * np.sin(2 * theta_rad)
                    )
                ...
                if C_min > C:
                    ...
                    logger.debug(
                        "Predicted theta_min lead us to a curvature maximum "
                ...
                if interpolate:
                    f1 = (
                        np.sin(rad_trial - rad_min) / np.sin(rad_trial) * f1
                        + np.sin(rad_min) / np.sin(rad_trial) * f1_trial
                        + (
                            1
                            - np.cos(rad_min)
                            - np.sin(rad_min) * np.tan(rad_trial / 2)
                        )
                        * f0
                    )
                else:
        ...
        sys.stdout.flush()
    print(
        f"@Did {tot_rot_force_evals} force evaluations in the rotation steps "
        f"using the {opt_name_dict[rot_opt]} optimizer."
    )
    tot_force_evals = tot_rot_force_evals + add_force_evals
    print(
        f"@Used {add_force_evals} additional force evaluations for a total of "
        f"{tot_force_evals} force evaluations."
    )

    dimer_results = DimerResult(
        dimer_cycles=dimer_cycles,

...
class AnimPlot:
    def __init__(self, coords, tangents, cycles, ge):
        ...
        # Tangents
        self.tangent_quiv = self.ax.quiver(
            images_x[1:-1], images_y[1:-1], tangents_x, tangents_y, color="b"
        )
    ...
    def animate(self):
        self.animation = animation.FuncAnimation(
            self.fig, self.func, frames=range(self.cycles), interval=250
        )
        plt.show()
    ...
    for i in range(max_cycles):
        ...
        for j in range(1, len(coords) - 1):
            ...
            if next_energy > jth_energy > prev_energy:
                ...
                max_energy_diff = max(
                    abs(next_energy - jth_energy), abs(prev_energy - jth_energy)
                )
                min_energy_diff = min(
                    abs(next_energy - jth_energy), abs(prev_energy - jth_energy)
                )
                ...
                if next_energy > prev_energy:
                    tangent = (
                        left_coords_diff * max_energy_diff
                        + right_coords_diff * min_energy_diff
                    )
                else:
                    tangent = (
                        left_coords_diff * min_energy_diff
            ...
            parallel_forces = (
                k
                * (
                    np.linalg.norm(next_coords - jth_coords)
                    - np.linalg.norm(prev_coords - jth_coords)
                )
                * tangent
            )
            perpendicular_gradient = (
                -forces[:, j] + np.dot(forces[:, j], tangent) * tangent
            )

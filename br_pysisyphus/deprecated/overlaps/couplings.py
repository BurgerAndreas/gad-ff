...
def run():
    ...
    consider_first = 3
    ens_sorted, inds_sorted = sort_cut(
        ens, max_ovlp_inds, consider_first=consider_first
    )
...
def couplings(states):
    ...
    energies_sorted, inds_sorted = sort_cut(
        energies, max_ovlp_inds, consider_first=consider_first
    )
    # fig, ax = plt.subplots()
    # ax.plot(ens_sorted, "o-")
    # plt.show()
    state_couplings = {
        (ind_a, ind_b): get_state_couplings(energies, energies_sorted, ind_a, ind_b)

...
from pysisyphus.helpers import geom_from_xyz_file, index_array_from_overlaps, np_print
...
class Overlapper:
    ...
    def __init__(
        self, path, ovlp_with="previous", prev_n=0, calc_key=None, calc_kwargs=None
    ):
        self.path = Path(path)
    ...
    def discover_files(self, path):
        ...
        if len(files) > 1:
            ...
            base_str = calc_str
        print(
            f"Found {len(files)} files starting with '{base_str}'. "
    ...
    def discover_files_in_dir(self, path, exts):
        ...
        for ext in exts:
            ...
            assert len(fns) == 1, (
                f"Searched for *.{ext} and was expecting "
    ...
    def set_files_on_calculator(
        self, geom, files_dict, calc_class, exts, calc_number, cycle_number=0
    ):
    ...
    def set_files_on_calculators(self, geoms, files_dict, calc_class, exts):
        for i, geom in enumerate(geoms):
            calc_number, cycle_number = i, 0
            self.set_files_on_calculator(
                geom, files_dict, calc_class, exts, calc_number, cycle_number
            )
    ...
    def set_gaussian16_files_from_dir(self, geom, path, calc_number):
        ...
        self.set_files_on_calculator(
            geom, files_dict, Gaussian16, exts_without_log, calc_number
        )
    ...
    def similar_overlaps(self, overlaps_for_state, ovlp_thresh=0.1, diff_thresh=0.2):
    ...
    def get_ovlp_func(
        self, ovlp_type, double_mol=False, recursive=False, consider_first=None
    ):
        ...
        def tden_ovlp(calc1, calc2, ao_ovlp):
            return calc1.tdens_overlap_with_calculator(calc2, ao_ovlp=ao_ovlp)
        ...
        def ovlp_func(geoms, i, j, depth=2, ao_ovlp=None):
            ...
            if similar:
                self.log(
                    "Some entries of the overlap matrix between steps "
    ...
    def overlaps_for_geoms(
        ...
        # "together."
        # )
        ovlp_func = self.get_ovlp_func(ovlp_type, double_mol, recursive, consider_first)
        ...
        inds_list = list()
        ovlp_mats = list()
        is_similar = lambda ovlp_mat: any(
            [
                self.similar_overlaps(per_state)
                for per_state in ovlp_mat[:, :consider_first]
            ]
...
if __name__ == "__main__":
    ...
    path = Path(
        "/scratch/programme/pysisyphus/tests_staging/test_diabatizer/cb3_def2svp/first_five"
    )
    calc_kwargs = {

...
class Plotter:
    def __init__(self, coords, data, ylabel, interval=750, save=None, legend=None):
        self.coords = coords
        self.data = data
        self.ylabel = ylabel
    ...
    def animate(self):
        ...
        self.animation = FuncAnimation(
            self.fig, self.update_func, frames=self.cycles, interval=self.interval
        )
        if self.save:
...
def plot_params(inds):
    ...
    assert all(
        [len(inds_list[i]) == len(inds_list[i + 1]) for i in range(len(inds_list) - 1)]
    ), (
        "Can only display "
...
def parse_args(args):
    ...
    parser.add_argument(
        "--first", type=int, help="Only consider the first [first] cycles."
    )
    parser.add_argument(
        "--last", type=int, help="Only consider the last [last] cycles."
    )
    ...
    group.add_argument("--energies", "-e", action="store_true", help="Plot energies.")
    group.add_argument("--aneb", action="store_true", help="Plot Adaptive NEB.")
    group.add_argument(
        "--all_energies",
        "-a",
        action="store_true",

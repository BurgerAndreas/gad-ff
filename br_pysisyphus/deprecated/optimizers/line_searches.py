...
def interpol_alpha_cubic(f_0, df_0, f_alpha_0, f_alpha_1, alpha_0, alpha_1):
    ...
    B = np.array(
        ((f_alpha_1 - f_0 - df_0 * alpha_1), (f_alpha_0 - f_0 - df_0 * alpha_0))
    )
    a, b = quot * A @ B
            ...
            **kwargs,
            ...
            def get_phi_dphi(what, alpha, check=True):
                ...
                if (
                    check
                    and (alpha > 0.0)
                    and (alpha in alpha_fs)
                    and (alpha in alpha_gs)
                    and cond_func(alpha)
                ):
            ...
            def strong_wolfe_condition(alpha):
                ...
                return sufficiently_decreased(alpha) and strong_curvature_condition(
                    alpha
                )
            ...
            linesearch_result = func(
                x0, p, get_phi_dphi, get_fg, conds, max_cycles, *args, **kwargs
            )
...
def hager_zhang(
    ...
    epsk = eps * abs(get_fg("f", 0.0))
    phi0, dphi0 = get_phi_dphi("fg", 0.0)
    f0, g0 = get_fg("fg", 0.0)
    ...
    try:
        if quad_step:
            g0_ = (
                -2 * abs(get_fg("f", 0) / alpha_init)
                if (dphi0_prev is None)
                else dphi0_prev
            )
    ...
    def zoom(alpha_lo, alpha_hi, phi_lo, phi_alpha_=None, alpha_0_=None, max_cycles=10):
        ...
        for j in range(max_cycles):
            ...
            if len(phi_alphas) > 1:
                ...
                alpha_j = interpol_alpha_cubic(
                    phi0, dphi0, phi_alpha_, phi_alpha_prev, alpha_0_, alpha_prev
                )

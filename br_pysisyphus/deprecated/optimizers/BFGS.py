...
class BFGS(BacktrackingOptimizer):
    def __init__(self, geometry, alpha=1.0, bt_force=20, **kwargs):
        ...
        if hasattr(self.geometry, "internal") and (self.geometry.internal is not None):
            raise Exception("Have to add hessian projections etc.")
        self.log(
            "BFGS with align=True is somewhat broken right now, so "
    ...
    def optimize(self):
        ...
        rho = 1.0 / y.dot(sigma)
        if (
            (np.array_equal(self.inv_hessian, self.eye))
        ...
        self.inv_hessian = A.dot(self.inv_hessian).dot(B) + np.outer(sigma, sigma) * rho

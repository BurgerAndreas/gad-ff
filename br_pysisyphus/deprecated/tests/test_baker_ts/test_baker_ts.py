...
def run_baker_ts_opts(geoms, meta, coord_type="cart", thresh="baker", runid=0):
    ...
    for i, (name, geom) in enumerate(geoms.items()):
        ...
        geom.calc = Gaussian16(route="HF/3-21G", **calc_kwargs)
        geom = augment_bonds(geom)
        # geom.calc = PySCF(basis="321g", **calc_kwargs)
        ...
        try:
            ...
            print(
                green(
                    f"\t@Energies MATCH for {name}! ({geom.energy:.6f}, {ref_energy:.6f})"
                )
            )
            ...
            print(
                red(
                    f"\t@Calculated energy {geom.energy:.6f} and reference "
...
def _test_baker_ts_optimizations():
    ...
    for i in range(runs):
        ...
        results, duration, cycles = run_baker_ts_opts(
            geoms, meta, coord_type, thresh, runid=i
        )
    ...
    cycles = {name: [result[name][0] for result in all_results] for name in names}

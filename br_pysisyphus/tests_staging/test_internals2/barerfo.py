...
def rfo(gradient, H, trust=0.3):
    H_aug = np.bmat(((H, gradient[:, None]), (gradient[None, :], [[0]])))
...
def run_bare_rfo(xyz_fn, charge, mult, trust=0.3, max_cycles=150):
    ...
    geom.calc = XTB(pal=4, charge=charge, mult=mult)
    # geom.calc = Gaussian16(pal=3, route="HF 3-21G", charge=charge, mult=mult))
    grads = list()
    steps = list()
    ...
    for i in range(max_cycles):
        ...
        print(
            f"{i:02d}: max(f)={max_g:.6f}, rms(f)={rms_g:.6f}, "
...
def run():
    ...
    for xyz_fn, (charge, mult) in GEOMS.items():
        ...
        try:
            ...
            cycles = 0
        # geom = geom_from_library(full_xyz, coord_type="redund")
        # geom.calc = XTB(pal=2, charge=charge, mult=mult)

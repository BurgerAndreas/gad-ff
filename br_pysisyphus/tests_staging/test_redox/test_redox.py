...
RedoxResult = namedtuple(
    "RedoxResult", "energy_gas hessian_gas energy_solv dG_solv geom_gas charge mult"
)
...
def run_calculations(geom, charge, mult, calc_getter_gas, calc_getter_solv, opt=False):
    ...
    energyg = geom.energy
    save_hessian(
        f"{get_name('gas')}.h5", geom, cart_hessian=Hg, energy=energyg, mult=mult
    )
...
def test_redox(inp_1, inp_2, calc_cls, gas_kwargs, solv_kwargs):
    ...
    result_1 = run_calculations(
        geom_1, charge_1, mult_1, calc_getter_gas, calc_getter_solv, opt=opt_1
    )
    print()
    result_2 = run_calculations(
        geom_2, charge_2, mult_2, calc_getter_gas, calc_getter_solv, opt=opt_2
    )

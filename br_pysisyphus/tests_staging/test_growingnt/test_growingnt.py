...
def test_anapot_growingnt():
    ...
    eps = 0.05
    damp = 0.05
    gnt = GrowingNT(get_geoms(coords, calc_getter), calc_getter, eps=eps, damp=damp)
...
def test_mullerbrown_growingnt():
    ...
    gnt = GrowingNT(
        get_geoms(coords, calc_getter), calc_getter, eps=eps, damp=damp, max_nodes=23
    )
    gnt.run()
    plot(gnt, calc_getter(), levels)

...
def rattle_closure(
    ...
    def rattle(coords, velocities, forces):
        ...
        coords3d_updated = (coords + dt * velocities + dt2_sq * acceleration).reshape(
            -1, 3
        )
        ...
        velocities3d_updated += dt2 * (forces_new * inv_masses_rep * FORCE2ACC).reshape(
            -1, 3
        )
        if remove_com_v:
            velocities3d_updated = remove_com_velocity(velocities3d_updated, masses)
        ...
        for i in range(max_cycles):
            ...
            for j, (atom_1, atom_2) in enumerate(constraints):
                velocity_diff = (
                    velocities3d_updated[atom_1] - velocities3d_updated[atom_2]
                )
        ...
        return (
            coords3d_updated.flatten(),
            velocities3d_updated.flatten(),
            energy_new,
            forces_new,
        )

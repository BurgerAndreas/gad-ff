...
def lincs_closure(geom, constraints, order=4):
    ...
    for i, con_1 in enumerate(constraint_sets):
        for j in range(c_max):
            conns_ij = conns[i, j]
            sign = signs[
                (atom_1[i] == atom_1[conns_ij]) or (atom_2[i] == atom_2[conns_ij])
            ]
            # Connected atom
    ...
    def lincs(prev_coords3d, new_coords3d):
        ...
        for i, (a1, a2) in enumerate(constraints):
            ...
            rhs[0, i] = S_diag[i] * (
                (B[i] * (new_coords3d[a1] - new_coords3d[a2])).sum() - lengths[i]
            )
            sol[i] = rhs[0, i]
        ...
        for i, (a1, a2) in enumerate(constraints):
            length_i = lengths[i]
            p = (
                2 * length_i**2 - (-((new_coords3d[a1] - new_coords3d[a2]) ** 2)).sum()
            ) ** 0.5

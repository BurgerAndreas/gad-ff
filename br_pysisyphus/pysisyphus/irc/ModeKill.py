...
class ModeKill(IRC):
    def __init__(
        ...
        **kwargs,
        ...
        # ent prematurely.
        super().__init__(
            geometry, downhill=True, rms_grad_thresh=1e-6, hessian_init="calc", **kwargs
        )
    ...
    def step(self):
        if self.hessian_update and (self.cur_cycle > 0):
            ...
            self.log(
                f"Did {key} hessian update: norm(dx)={norm_dx:.4e}, "
        ...
        for i, ovlps in enumerate(overlaps):
            ...
            ovlp_str = " ".join(
                [
                    f"{i:02d}: {o:.4f}"
                    for i, o in zip(self.indices[above_thresh], ovlps[above_thresh])
                ]
            )

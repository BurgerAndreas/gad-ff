...
def match_geoms(ref_geom, geom_to_match, hydrogen=False):
    ...
    for atom in atoms:
        ...
        logging.debug("col_inds", col_inds)
        old_inds = inds_to_match[atom]
        new_inds = old_inds[col_inds]
        logging.debug("old_inds", old_inds)
        logging.debug("new_inds", new_inds)
        new_coords_for_atom = coords_to_match_for_atom[new_inds]
        # logging.debug(ref_coords_for_atom)
        # logging.debug(new_coords_for_atom)
        # logging.debug(ref_coords_for_atom-new_coords_for_atom)
        # Update coordinates
        logging.debug("old coords")
...
def check_for_end_sign(check_user=True, cwd=".", add_signs=None):
    ...
    for sign in signs:
        ...
        if sign_path.exists() and sign_owner(sign_path):
            logging.debug(f"Found sign '{sign}'. Ending run.")
            os.remove(sign_path)
            sign_found = sign
...
def do_final_hessian(
    ...
    logging.debug(highlight_text("Hessian at final geometry", level=1))
    logging.debug()
    ...
    logging.debug()
    logging.debug("First 10 eigenvalues", eigval_str)
    if neg_num > 0:
        wavenumbers = eigval_to_wavenumber(neg_eigvals)
        wavenum_str = np.array2string(wavenumbers, precision=2)
        logging.debug("Imaginary frequencies:", wavenum_str, "cm⁻¹")
    ...
    if save_hessian:
        ...
        logging.debug(f"Wrote Hessian data HD5 file '{final_h5_hessian_fn}'.")
...
def print_barrier(ref_energy, comp_energy, ref_str, comp_str):
    ...
    logging.debug(f"Barrier between {ref_str} and {comp_str}: {barrier:.1f} kJ mol⁻¹")
    return barrier

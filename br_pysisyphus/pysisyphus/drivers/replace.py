...
def replace_atoms(geom, replacements, opt=False, use_xtb=False, charge=0, mult=1):
    ...
    for ind, repl_ind, repl_geom in replacements:
        repl_frag = replace_atom(
            geom, ind, repl_geom, repl_ind, False, opt=opt, use_xtb=use_xtb
        )
...
def run():
    ...
    union = replace_atoms(
        geom, replacements, opt=opt, charge=charge, mult=mult, use_xtb=xtb
    )
    print(union.as_xyz())

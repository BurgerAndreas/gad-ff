...
try:
    ...
    print("\ngpu4pyscf.drivers.dft_3c_driver import failed:")
    print(e)
    traceback.print_exc()
    print(
        'Either you don\'t have a GPU, so cupy failed with "CUDA driver version is insufficient for CUDA runtime version"'
    )
    print(
        "Or GPU4PySCF version is lower than 1.3.1, so import failed with \"No module named 'gpu4pyscf.drivers.dft_3c_driver'\""
    )
    print(
        "Or some other problem occurs when trying to load parse_3c() function from gpu4pyscf."
    )
...
class PySCF(OverlapCalculator):
    ...
    def get_driver(self, step, mol=None, mf=None):
        ...
        if self.solvation_model:
            if self.solvation_model in ["IEF-PCM", "C-PCM", "SS(V)PE", "COSMO"]:
                ...
                print(
                    f"Solvation model {self.solvation_model} is not supported in GPU4PySCF, treat as Null"
                )
    ...
    def prepare_mol(self, atoms, coords, build=True):
        ...
        if self.parameters_3c is not None:
            ...
            if self.basis != basis:
                warnings.warn(
                    f"The basis provided in the input ({self.basis}) is not the required basis ({basis}) for the 3c method. "
    ...
    def get_forces(self, atoms, coords, **prepare_kwargs):
        ...
        if self.parameters_3c is not None:
            pyscf_xc, nlc, basis, ecp, (xc_disp, disp), xc_gcp = self.parameters_3c
            grad_driver.get_dispersion = MethodType(
                gen_disp_grad_fun(xc_disp, xc_gcp), grad_driver
            )
    ...
    def get_hessian(self, atoms, coords, **prepare_kwargs):
        ...
        if self.parameters_3c is not None:
            pyscf_xc, nlc, basis, ecp, (xc_disp, disp), xc_gcp = self.parameters_3c
            hessian_driver.get_dispersion = MethodType(
                gen_disp_hess_fun(xc_disp, xc_gcp), hessian_driver
            )
        ...
        for i, step in enumerate(steps):
            if i == 0:
                ...
                if self.parameters_3c is not None:
                    ...
                    pyscf_xc, nlc, basis, ecp, (xc_disp, disp), xc_gcp = (
                        self.parameters_3c
                    )
                    mf.xc = pyscf_xc
                    mf.nlc = nlc

    ...
    def get_molecule(self, atoms, coords):
        ...
        if self.program == "openmm":
            if self.connectivity is None:
                self.log(
                    "No connectivity specified! Using hacky connectivity "
                    f"guess with bond-order={self.bond_order}"
                )
                connectivity = qcel.molutil.guess_connectivity(
                    atoms, c3d, threshold=1.3
                )
                self.connectivity = [
                    (at1, at2, self.bond_order) for at1, at2 in connectivity
                ]

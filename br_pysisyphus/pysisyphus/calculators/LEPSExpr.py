...
class LEPSExpr:
    ...
    def V_LEPS(self, x=None, y=None, abc=None):
        ...
        Qab, Qbc, Qac = [self.Q(d, self.alpha, self.r0, r) for d, r in zip(self.ds, rs)]
        Jab, Jbc, Jac = [self.J(d, self.alpha, self.r0, r) for d, r in zip(self.ds, rs)]

        first_term = Qab / (1 + a) + Qbc / (1 + b) + Qac / (1 + c)
        second_term = (
            Jab**2 / (1 + a) ** 2 + Jbc**2 / (1 + b) ** 2 + Jac**2 / (1 + c) ** 2
        )
        third_term = (
            -Jab * Jbc / ((1 + a) * (1 + b))
            - Jbc * Jac / ((1 + b) * (1 + c))
    ...
    def V_dimer(self):
        ...
        x, y = symbols("x y")
        #           Ai , x0i    ,  y0i     , sxi, syi
        params = ((-1.5, 2.02083, -0.172881, 0.1, 0.35), (6.0, 0.8, 2.0, 0.5, 0.7))

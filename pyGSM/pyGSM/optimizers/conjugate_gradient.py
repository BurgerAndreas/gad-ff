...
class conjugate_gradient(base_optimizer):
    ...
    def optimize(
        self, molecule, refE=0.0, opt_type="UNCONSTRAINED", opt_steps=3, ictan=None
    ):
        ...
        if molecule.gradrms < self.conv_grms:
            nifty.logger.debug(" already at min")
        ...
        for ostep in range(opt_steps):
            ...
            if self.initial_step is True:
                ...
                nifty.logger.debug(" beta = %1.2f" % beta)
            ...
            if actual_step > self.options["DMAX"]:
                ...
                nifty.logger.debug(" reducing step, new step = %1.2f" % step)
            else:
                step = actual_step
            ...
            #    nifty.logger.debug('[INFO] converged')

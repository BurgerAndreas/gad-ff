...
def eckart_align(geom1, geom2, masses, rfrac, max_iter=200):
    ...
    COMproduct = np.sum(
        manage_xyz.xyz_to_np(geom2) * np.outer(masses, [1.0] * 3), 0
    ) / np.sum(masses)
    ...
    mwcproduct = (
        xyzproduct * units.ANGSTROM_TO_AU * np.outer(np.sqrt(masses), [1.0] * 3)
    )
    thetas = np.zeros(3)
    ...
    for i in range(maxiter):
        ...
        if gradmag < tol and all(hess_evals > 0.0):
            break
...
def calc_rot_vec_diff(v1_in, v2_in):
    ...
    #     nifty.logger.debug("in : v1= % 8.4f % 8.4f % 8.4f v2= % 8.4f % 8.4f % 8.4f" % (v1_in[0], v1_in[1], v1_in[2], v2_in[0], v2_in[1], v2_in[2]))
    #     nifty.logger.debug("out: v1= % 8.4f % 8.4f % 8.4f v2= % 8.4f % 8.4f % 8.4f" % (v1[0], v1[1], v1[2], v2[0], v2[1], v2[2]))
    # nifty.logger.debug("")
    return v1 - v2

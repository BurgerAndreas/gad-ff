...
try:
    ...
    nifty.logger.warning(
        "NetworkX cannot be imported (topology tools won't work).  Most functionality should still work though."
    )
...
def AtomContact(xyz, pairs, box=None, displace=False):
    ...
    # nifty.logger.debug("sel1, sel2")
    # nifty.logger.debug(sel1)
    # nifty.logger.debug(sel2)
...
class Topology:
    ...
    def build_topology(
        ...
        **kwargs
        ...
        if hybrid_indices is None:
            ...
            # nifty.logger.debug("non-cartesian indices")
            # nifty.logger.debug(primitive_indices)
        ...
        # nifty.logger.debug(len(fragments))
        # for frag in fragments:
        #    nifty.logger.debug(frag.L())

        # nifty.logger.debug("nodes of Graph")
        # nifty.logger.debug(topology.L())
    ...
    def build_bonds(xyz, atoms, primitive_indices, prim_idx_start_stop=None, **kwargs):
        ...
        if use_grid and prim_idx_start_stop is None:
            ...
            # nifty.logger.debug(first_list)

            second_list = []
            for tup in prim_idx_start_stop:
                for i in range(tup[0], tup[1]):
                    second_list.append(list(range(i + 1, tup[1] + 1)))
            # nifty.logger.debug(second_list)
        ...
        # nifty.logger.debug('bond list')
        # nifty.logger.debug(bondlist)
    ...
    def read_bonds_from_file(filename):
        ...
        nifty.logger.debug(sorted_bonds[:10])
...
if __name__ == "__main__" and __package__ is None:
    ...
    # for frag in fragments:
    #    nifty.logger.debug(frag.L())
    ##nifty.logger.debug(len(mytop.fragments))
    ##nifty.logger.debug(mytop.fragments)
    ...
    #            new=True
    #            hybrid_idx_start_stop.append((start,end))
    # nifty.logger.debug(hybrid_idx_start_stop)

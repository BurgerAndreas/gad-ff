...
from utilities import manage_xyz, elements, options, block_matrix, nifty
...
class Molecule(object):
    ...
    def __init__(self, options, **kwargs):
        ...
        #    nifty.logger.debug(" building coordinate object")
    ...
    def mass_amu_triples(self):
        return np.array(
            [[ele.mass_amu, ele.mass_amu, ele.mass_amu] for ele in self.atoms]
        )
    ...
    def center_of_mass(self):
        ...
        return np.sum(
            [self.xyz[i, :] * atomic_masses[i] / M for i in range(self.natoms)], axis=0
        )
    ...
    def mass_weighted_cartesians(self):
        M = self.total_mass_au
        return np.asarray(
            [self.xyz[i, :] * self.atomic_mass[i] / M for i in range(self.natoms)]
        )
    ...
    def radius_of_gyration(self):
        ...
        xyz1 -= com
        return (
            np.sum(self.atomic_mass[i] * np.dot(x, x) for i, x in enumerate(xyz1)) / M
        )
    ...
    def gradx(self):
        return np.reshape(
            self.PES.get_gradient(self.xyz, frozen_atoms=self.frozen_atoms), (-1, 3)
        )

    @property
    def gradient(self):
        gradx = self.PES.get_gradient(self.xyz, frozen_atoms=self.frozen_atoms)
        return self.coord_obj.calcGrad(
            self.xyz, gradx
        )  # CartesianCoordinate just returns gradx
    ...
    def form_Hessian_in_basis(self):
        ...
        self.Hessian = block_matrix.dot(
            block_matrix.dot(
                block_matrix.transpose(self.coord_basis), self.Primitive_Hessian
            ),
            self.coord_basis,
        )
        return self.Hessian
    ...
    def update_xyz(self, dq=None, verbose=True):
        # print " updating xyz"
        if dq is not None:
            self.xyz = self.coord_obj.newCartesian(
                self.xyz, dq, frozen_atoms=self.frozen_atoms, verbose=verbose
            )
        return self.xyz

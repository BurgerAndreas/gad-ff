...
class nanoreactor_engine(Lot):
    ...
    def run(self, geom, mult, ad_idx, runtype="gradient"):
        ...
        # nifty.logger.debug(self.options['job_data']['orbfile']['propagate'])
        if (
            self.hasRanForCurrentCoords == False
            and self.node_id in self.options["job_data"]["orbfile"].keys()
        ):
            nifty.logger.debug("Recalculating energy")
        ...
        if self.node_id not in self.options["job_data"]["orbfile"].keys():
            if "copied_orb" in self.options["job_data"]["orbfile"].keys():
                if (
                    self.options["job_data"]["orbfile"]["copied_orb"]
                    in self.options["job_data"]["orbfile"].keys()
                ):
                ...
                orb_guess = None
            if (self.node_id - 1) in self.options["job_data"][
                "orbfile"
            ].keys() and orb_guess == None:
                orb_guess = self.options["job_data"]["orbfile"][self.node_id - 1]
            elif (self.node_id + 1) in self.options["job_data"][
                "orbfile"
            ].keys() and orb_guess == None:
                orb_guess = self.options["job_data"]["orbfile"][self.node_id + 1]
        else:
        ...
        try:
            if orb_guess:
                results = self.engine.compute_blocking(
                    xyz, fields, job_type="gradient", guess=orb_guess
                )  # compute__(fields = "energy, gradient, orbfiles")
            # if we're not using a previous orbital as a guess, we want to ensure we find the correct SCF minimum if using guess = generate
                ...
                nifty.logger.debug("No orbital guess available")
        ...
        if self.engine.options["closed_shell"]:
            ...
            self.options["job_data"]["orbfile"].update(
                {self.node_id: orb_a_path + " " + orb_b_path}
            )
...
if __name__ == "__main__":
    ...
    nifty.logger.debug(test_lot.get_energy(xyz, 1, 0))

    nifty.logger.debug("getting grad")
    nifty.logger.debug(test_lot.get_gradient(xyz, 1, 0))

...
class BAGEL(Lot):
    ...
    def __init__(self, options):
        ...
        self.file_options.set_active(
            ...
            bool,
            "",
            depend=(self.file_options.casscf),
            msg="casscf must be True",
        )
        self.file_options.set_active(
        ...
        if "caspt2" not in self.file_options.ActiveOptions:
            for key in ["ms", "xms", "sssr", "shift", "frozen"]:
                nifty.logger.debug("deactivating key ", key)
                keys_to_del.append(key)
        for key in keys_to_del:
            self.file_options.deactivate(key)
        ...
        self.file_options.set_active(
            ...
            str,
            doc="guess",
            depend=(os.path.isfile(guess_file + ".archive")),
            msg="ref does not exist or not needed, deactivating for now...",
        )
    ...
    def copy(cls, lot, options, copy_wavefunction=True):
        ...
        nifty.logger.debug(" making folder scratch/{}".format(node_id))
    ...
    def write_input(self, geom, runtype="gradient"):
        ...
        if "casscf" in self.file_options.ActiveOptions:
            inpfile.write(
                (
                    ...
                    ' "maxiter": {} \n'
                    "  }},\n\n".format(
                        self.nstate, self.nact, self.nclosed, self.maxiter
                    )
                )
            )
        else:
            nifty.logger.debug(" only casscf implemented now")
        ...
        if runtype == "gradient":
            ...
            inpfile.write(
                '     {{ "title"  : "nacme", "target1" : {}, "tartet2": {}, "nacmtype" : "full" }}\n'.format(
                    self.coupling_states[0], self.coupling_states[1]
                )
            )
            inpfile.write(" ],\n")

        if "caspt2" in self.file_options.ActiveOptions and (
            runtype == "gh" or runtype == "gradient"
        ):
            inpfile.write(
                (
                    ' "method" : [ {{\n'
            ...
            nifty.logger.debug("only caspt2 gradient implemented")
            raise NotImplementedError
    ...
    def parse(self, geom, runtype="gradient"):
        ...
        if "caspt2" in self.file_options.ActiveOptions:
            ...
            nifty.logger.debug("CASSCF not currently enabled")
            ...
            with open(tempfileout, "r") as f:
                for line in f:
                    if line.startswith("* Nuclear energy gradient", 2) and count < 3:
                        ...
                        line.startswith("* Nuclear energy gradient", 2)
                        and count == 3
                        and runtype == "gh"
                    ):
                        next(f)
    ...
    def runall(self, geom, runtype=None):
        ...
        if (
            not self.gradient_states and not self.coupling_states
        ) or runtype == "energy":
...
if __name__ == "__main__":
    ...
    nifty.logger.debug(E0, E1)
    nifty.logger.debug(g0.T)
    nifty.logger.debug(g1.T)
    nifty.logger.debug(c.T)

    # for line in B.file_options.record():
    #    nifty.logger.debug(line)

    # for key,value in B.file_options.ActiveOptions.items():
    #    nifty.logger.debug(key,value)

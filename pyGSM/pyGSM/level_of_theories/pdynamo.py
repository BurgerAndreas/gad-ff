...
class pDynamo(Lot):
    ...
    def build_system(self):
        ...
        if self.use_orca:
            ...
            nifty.logger.debug(parsed_keywords)

            qcmodel = pM.QCModel.QCModelORCA.WithOptions(
            ...
            qcmodel.electronicState = pM.QCModel.ElectronicState.WithOptions(
                charge=self.charge, multiplicity=self.states[0][0]
            )
            ...
            electronicState = pM.QCModel.ElectronicState.WithOptions(
                charge=self.charge, multiplicity=self.states[0][0]
            )
            qcModel = pM.QCModel.QCModelDFTB.WithOptions(
                deleteJobFiles=False,
    ...
    def run(self, geom, multiplicity=1):
        ...
        nifty.logger.debug(energy)

        gradient = []
        for i in range(len(geom)):
            for j in range(3):
                gradient.append(
                    self.system.scratch.gradients3[i, j]
                    * units.KJ_MOL_TO_AU
                    / units.ANGSTROM_TO_AU
                )  # Ha/Bohr
        gradient = np.asarray(gradient)
        # nifty.logger.debug(gradient)

...
class SE_Cross(SE_GSM):
    ...
    def go_gsm(self, max_iters=50, opt_steps=3, rtype=0):
        ...
        nifty.logger.debug(" Initial energy is {:1.4f}".format(self.nodes[0].energy))
        sys.stdout.flush()
        ...
        nifty.logger.debug(" Initial bdist is %1.3f" % self.nodes[0].bdist)
        ...
        nifty.logger.debug(" SE_Cross growth phase over")
        nifty.logger.debug(" Warning last node still not fully optimized")
        ...
        if True:
            ...
            nifty.logger.debug(
                " extra constrained optimization for the nnR-1 = %d" % (self.nR - 1)
            )
            self.optimizer[self.nR - 1].conv_grms = self.options["CONV_TOL"] * 5
            ictan = self.get_tangent_xyz(
                self.nodes[self.nR - 1].xyz,
                self.nodes[self.nR - 2].xyz,
        ...
        self.xyz_writer(
            "after_penalty_{:03}.xyz".format(self.ID),
            self.geometries,
            self.energies,
            self.gradrmss,
            self.dEs,
        ...
        if rtype == 0:
            ...
            nifty.logger.debug(
                " sigma for node %d is %.3f" % (self.nR, self.nodes[self.nR].PES.sigma)
            )
            path = os.path.join(
                os.getcwd(), "scratch/{:03d}/{}".format(self.ID, self.nR)
            )
        ...
        if self.optimizer[self.nR].converged:
            ...
            for n in range(self.nnodes):
                self.nodes[n].PES = PES.create_pes_from(
                    self.nodes[n].PES.PES2, options={"gradient_states": [(1, 1)]}
                )
            ...
            if deltaE > 20:
                ...
                nifty.logger.debug(" deltaE s1-minimum and MECI %5.4f" % deltaE)
                try:
    ...
    def check_if_grown(self):
        ...
        condition1 = abs(self.nodes[self.nR - 1].bdist) <= (1 - self.BDIST_RATIO) * abs(
            self.nodes[0].bdist
        ) and (abs(pes1dE) > abs(pes2dE))
        # condition2 = ((self.nodes[self.nR-1].bdist+0.1 > self.nodes[self.nR-2].bdist) and (1-self.BDIST_RATIO)*abs(self.nodes[0].bdist))
        if condition1:
            nifty.logger.debug(" Condition 1 satisfied")
            nifty.logger.debug(
                " bdist current %1.3f" % abs(self.nodes[self.nR - 1].bdist)
            )
            nifty.logger.debug(
                " bdist target %1.3f"
                % (abs(self.nodes[0].bdist) * (1 - self.BDIST_RATIO))
            )
            nifty.logger.debug(" Growth-phase over")
            isDone = True
        # elif condition2:
        #    nifty.logger.debug(" Condition 2 satisfied")
        #    nifty.logger.debug(" Growth-phase over")
        #    isDone = True
        return isDone

...
class Avg_PES(PES):
    ...
    def symmetric_orthogonalization(self, xyz):
        ...
        # nifty.logger.debug(" norm x %2.5f" % norm_dg)
        # nifty.logger.debug(" norm y %2.5f" % norm_nac)

        pitch = calc_pitch(dgrad, nac)
        # nifty.logger.debug(" pitch %1.6f" % pitch)
        ...
        P = ((sigma**2.0) / (1.0 - asymmetry**2.0)) * (
            1.0 - asymmetry * np.cos(2 * theta_s)
        )
        B = ((sigma**2) / (4 * asymmetry**2)) ** (1.0 / 3) * (
    ...
    def critical_points_bp(self, xyz, radius=0.2, num_slices=40):
        ...
        for n in range(num_slices):
            factorA = sigma * np.cos(theta[n] - theta_s) + np.sqrt(
                1.0 + asymmetry * np.cos(2 * theta[n])
            )
            factorB = sigma * np.cos(theta[n] - theta_s) - np.sqrt(
                1.0 + asymmetry * np.cos(2 * theta[n])
            )
        ...
        # for n in range(num_slices):
        #    nifty.logger.debug(" EB[%2i] = %2.8f" %(n,EB[n]))
        ...
        for n in theta_list:
            mxyz.append(
                xyz + radius * np.cos(theta[n]) * x + radius * np.cos(theta[n]) * y
            )
...
if __name__ == "__main__":
    ...
    nifty.logger.debug(pes.get_energy(coords))
    nifty.logger.debug(pes.get_gradient(coords))

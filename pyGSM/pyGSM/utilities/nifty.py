...
def encode(_l):
    return [[len(list(group)), name] for name, group in itertools.groupby(_l)]


def segments(e):
    # Takes encoded input.
    begins = np.array(
        [sum([k[0] for k in e][:j]) for j, i in enumerate(e) if i[1] == 1]
    )
...
def commadash(_l):
    ...
    return ",".join(
        "%i-%i" % (i[0] + 1, i[1]) if (i[1] - 1 > i[0]) else "%i" % (i[0] + 1)
        for i in segments(encode(LL))
    )
...
def printcool(
    ...
    def newlen(_l):
        return len(re.sub(r"\x1b\[[0-9;]*m", "", _l))
    ...
    for ln, line in enumerate(text):
        ...
        if ansi is not None:
            ...
            if color == 0 and bold:
                ...
                logger.log(
                    level,
                    "%s| %s " % (sym, padleft) + line + " %s |%s\n" % (padright, sym),
                )
            else:
                logger.log(
                    level,
                    "%s| \x1b[%s9%im%s " % (sym, bold and "1;" or "", color, padleft)
                    + line
                    + " %s\x1b[0m |%s\n" % (padright, sym),
                )
...
def printcool_dictionary(
    ...
    if isinstance(Dict, OrderedDict):
        logger.log(
            ...
            "\n".join(
                [
                    " " * leftpad + "%s %s " % (magic_string(str(key)), str(Dict[key]))
                    for key in Dict
        ...
        logger.log(
            ...
            "\n".join(
                [
                    " " * leftpad + "%s %s " % (magic_string(str(key)), str(Dict[key]))
                    for key in sorted([i for i in Dict])
                    if Dict[key] is not None
                ]
            ),
        )
...
def monotonic_decreasing(arr, start=None, end=None, verbose=False):
    ...
    idx = [start]
    if verbose:
        logger.info("Starting @ %i : %.6f" % (start, arr[start]))
    if end > start:
        i = start + 1
        while i < end:
            if arr[i] < a0:
                ...
                idx.append(i)
                if verbose:
                    logger.info("Including  %i : %.6f" % (i, arr[i]))
            else:
                if verbose:
                    logger.info("Excluding  %i : %.6f" % (i, arr[i]))
            i += 1
    if end < start:
        i = start - 1
        while i >= end:
...
def get_least_squares(x, y, w=None, thresh=1e-12):
    ...
    if w is not None:
        if len(w) != n_x:
            warn_press_key(
                "The weight array length (%i) must be the same as the number of 'X' data points (%i)!"
                % len(w),
                n_x,
            )
        w /= np.mean(w)
    ...
    return (
        np.array(Beta).flatten(),
        np.array(Hat),
        np.array(yfit).flatten(),
        np.array(MPPI),
    )
...
def statisticalInefficiency(A_n, B_n=None, fast=False, mintime=3, warn=True):
    ...
    while t < N - 1:
        ...
        C = sum(dA_n[0 : (N - t)] * dB_n[t:N] + dB_n[0 : (N - t)] * dA_n[t:N]) / (
...
def mean_stderr(ts):
    ...
    return np.mean(ts), np.std(ts) * np.sqrt(
...
def multiD_statisticalInefficiency(A_n, B_n=None, fast=False, mintime=3, warn=True):
    ...
    for col in range(n_col):
        if B_n is None:
            multiD_sI[:, col] = statisticalInefficiency(
                A_n[:, col], B_n, fast, mintime, warn
            )
        else:
            multiD_sI[:, col] = statisticalInefficiency(
                A_n[:, col], B_n[:, col], fast, mintime, warn
            )
    return multiD_sI
...
def createWorkQueue(wq_port, debug=True, name="geomeTRIC"):
    ...
    WORK_QUEUE = work_queue.WorkQueue(
        port=wq_port, catalog=True, exclusive=False, shutdown=False
    )
    ...
    for sec in range(numwaits):
        ...
        if task:
            ...
            if verbose:
                ...
                logger.info(
                    "Job name = " + task.tag + "command = " + task.command + "\n"
                )
            ...
            if task.result != 0:
                ...
                # Assume that we're only interested in printing jobs that last longer than a minute.
                if exectime > print_time:
                    logger.info(
        ...
        if verbose:
            logger.info(
                ...
                % (
                    wq.stats.workers_init,
                    wq.stats.workers_idle,
                    wq.stats.workers_busy,
                    wq.stats.workers_joined,
            ...
            logger.info(
                ...
                % (
                    wq.stats.tasks_running,
                    wq.stats.tasks_waiting,
                    wq.stats.tasks_dispatched,
...
def onefile(fnm=None, ext=None, err=False):
    ...
    if fnm is not None:
        if os.path.exists(fnm):
                ...
                if os.path.exists(fdest):
                    if not filecmp.cmp(fsrc, fdest):
                        logger.error(
                            ...
                            % (
                                os.path.join(os.getcwd(), os.path.basename(fnm)),
                                os.path.abspath(fnm),
                            )
                        )
                        raise RuntimeError
            ...
            warn_once(
                "File specified by %s does not exist - will try to autodetect .%s extension"
                % (fnm, ext)
            )
    answer = None
    cwd = os.getcwd()
    ...
    if len(ls) != 1:
        if err:
            ...
            warn_once(
                "Cannot find a unique file with extension .%s in %s (%i found; %s)"
                % (ext, cwd, len(ls), " ".join(ls)),
                warnhash="Found %i .%s files" % (len(ls), ext),
            )
    else:
        answer = os.path.basename(ls[0])
        warn_once(
            "Autodetected %s in %s" % (answer, cwd), warnhash="Autodetected %s" % answer
        )
    return answer
...
def listfiles(fnms=None, ext=None, err=False, dnm=None):
    ...
    if answer == [] and err:
        logger.error(
            "listfiles function failed to come up with a file! (fnms = %s ext = %s)"
            % (str(fnms), str(ext))
        )
        raise RuntimeError
...
specific_dct = dict(
    list(itertools.chain(*[[(j, i[1]) for j in i[0]] for i in specific_lst]))
)
...
def which(fnm):
    ...
    try:
        return os.path.split(
            os.popen("which %s 2> /dev/null" % fnm).readlines()[0].strip()
        )[0]
    except:
        return ""
...
def _exec(
    ...
    **kwargs
    ...
    with LineChunker(process_out) as out_chunker, LineChunker(
        process_err
    ) as err_chunker:
        while True:

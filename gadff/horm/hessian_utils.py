...
def get_smallest_eigen_from_batched_hessians(batch, hessians, n_smallest=2) -> tuple[list[torch.Tensor], list[torch.Tensor]]:
...
def compute_hessian_single_batch(batch, coords, energy, forces) -> torch.Tensor:
    ...
    hessian = torch.vmap(get_vjp, in_dims=0, out_dims=0, chunk_size=None)(
        I_N
    )[0]
...
def predict_eigen_from_batch(batch, model, n_smallest=2) -> tuple[torch.Tensor, torch.Tensor]:
...
def test_hessian_utils():
    ...
    for i, batch in enumerate(dataloader):
        ...
        smallest_eigenvals_full, smallest_eigenvecs_full = get_smallest_eigen_from_full_hessian(batch, full_hessian, n_smallest=2)
        ...
        smallest_eigenvals_batched, smallest_eigenvecs_batched = get_smallest_eigen_from_batched_hessians(batch, hessians, n_smallest=2)
    ...
    for i, batch in enumerate(dataloader):
        ...
        smallest_eigenvals_single, smallest_eigenvecs_single = get_smallest_eigen_from_batched_hessians(
            batch, hessian_single, n_smallest=2
        )
        
        # Compare with the first element from the batched computation
        max_diff_eigenval = torch.max(torch.abs(smallest_eigenvals_single[0] - smallest_eigenvals_batched[0]))
        max_diff_eigenvec = torch.max(torch.abs(smallest_eigenvecs_single[0] - smallest_eigenvecs_batched[0]))
    ...
    for i, batch in enumerate(dataloader):
        ...
        full_hessian = compute_full_hessian(coords=batch.pos, energy=energy, forces=forces)
        seigvals, seigvecs = get_smallest_eigen_from_full_hessian(batch, full_hessian, n_smallest=2)
    t1 = time.time()
    ...
    for i, batch in enumerate(dataloader):
        ...
        hessians = compute_hessian_batches(batch, batch.pos, energy, forces)
        seigvals, seigvecs = get_smallest_eigen_from_batched_hessians(batch, hessians, n_smallest=2)
    t1 = time.time()
    ...
    for i, batch in enumerate(dataloader):
        ...
        hessian_single = compute_hessian_single_batch(batch, batch.pos, energy, forces)
        seigvals, seigvecs = get_smallest_eigen_from_batched_hessians(batch, hessian_single, n_smallest=2)
    t1 = time.time()
    ...
    if max_batch_size is not None:
        print(f"Maximum batch size that can be processed using batchwise method: {max_batch_size}")
    else:
    ...
    if max_batch_size_postselect is not None:
        print(f"Maximum batch size that can be processed using postselect method: {max_batch_size_postselect}")
    else:
        print("Could not process any batch size without running out of memory (postselect method).")

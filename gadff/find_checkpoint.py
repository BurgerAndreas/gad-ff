...
def find_checkpoint_by_wandb_run_id(
    wandb_run_id: str, 
    project_name: str = None, 
    checkpoint_base_dir: str = "checkpoint"
) -> List[str]:
    """
    Find checkpoint directories containing a specific wandb run ID.
    
    Args:
        wandb_run_id: The wandb run ID to search for
        project_name: Optional project name to narrow the search
        checkpoint_base_dir: Base directory where checkpoints are stored
        
    Returns:
        List of checkpoint directory paths containing the wandb run ID
    """
    checkpoint_dirs = []
    
    if project_name:
        # Search within specific project
        search_pattern = os.path.join(checkpoint_base_dir, project_name, f"*{wandb_run_id}*")
    else:
        # Search across all projects
        search_pattern = os.path.join(checkpoint_base_dir, "*", f"*{wandb_run_id}*")
...
def find_checkpoint_files_by_wandb_run_id(
    wandb_run_id: str, 
    project_name: str = None,
    checkpoint_base_dir: str = "checkpoint"
) -> Dict[str, List[str]]:
    """
    Find all checkpoint files (.ckpt) within directories containing a specific wandb run ID.
    ...
    """
    checkpoint_dirs = find_checkpoint_by_wandb_run_id(wandb_run_id, project_name, checkpoint_base_dir)
...
def get_best_checkpoint_by_wandb_run_id(
    wandb_run_id: str, 
    project_name: str = None,
    checkpoint_base_dir: str = "checkpoint",
    ...
    """
    Find the best checkpoint file based on a metric for a specific wandb run ID.
    ...
    """
    checkpoint_files = find_checkpoint_files_by_wandb_run_id(wandb_run_id, project_name, checkpoint_base_dir)
        ...
        for file_path in files:
            ...
            try:
                ...
                if len(parts) >= 5:  # ff, epoch, val-totloss, val-MAE_E, val-MAE_F
                    ...
                    if (best_is_min and value < best_value) or (not best_is_min and value > best_value):
                        best_value = value
                        best_file = file_path

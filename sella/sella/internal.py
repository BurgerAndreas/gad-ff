...
from sella.linalg import (
    SparseInternalJacobian,
    SparseInternalHessian,
...
class Coordinate:
    ...
    def calc(self, atoms: Atoms) -> float:
        return float(self._eval0(atoms[self.indices].positions, **self.kwargs))

    def calc_gradient(self, atoms: Atoms) -> np.ndarray:
        return np.array(self._eval1(atoms[self.indices].positions, **self.kwargs))

    def calc_hessian(self, atoms: Atoms) -> jnp.ndarray:
        return np.array(self._eval2(atoms[self.indices].positions, **self.kwargs))
...
def _angle(pos: jnp.ndarray, tvecs: jnp.ndarray) -> float:
    ...
    return jnp.arccos(dx1 @ dx2 / (jnp.linalg.norm(dx1) * jnp.linalg.norm(dx2)))
...
class BaseInternals:
    ...
    def _cache_check(self) -> None:
        ...
        if self._lastpos is None or np.any(self.all_atoms.positions != self._lastpos):
            self._cache = dict()
    ...
    def jacobian(self) -> np.ndarray:
        ...
        if "jacobian" not in self._cache:
            atoms = self.all_atoms
            self._cache["jacobian"] = [np.array(c.calc_gradient(atoms)) for c in self]
        indices = []
        jacs = []
        for coord, jac, active in zip(self, self._cache["jacobian"], self._active_mask):
            if active:
    ...
    def hessian(self) -> np.ndarray:
        ...
        if "hessian" not in self._cache:
            atoms = self.all_atoms
            self._cache["hessian"] = [np.array(c.calc_hessian(atoms)) for c in self]
        ...
        for idx, vals, active in zip(
            ...
            if not active:
                continue
            hessians.append(
                SparseInternalHessian(self.natoms + self.ndummies, idx, vals.copy())
            )
    ...
    def add_dummy_to_internals(self, idx: int) -> None:
        ...
        for i, rot in enumerate(self.internals["rotations"]):
            if idx in rot.indices[:-1]:
                ...
                new_rot = Rotation(
                    new_indices, rot.axis, self.all_atoms[new_indices].positions
                )
                self.internals["rotations"][i] = new_rot

    def check_all_gradients(self, delta: float = 1e-4, atol: float = 1e-6) -> bool:
        success = True
        for coord in self:
    ...
    def fix_rotation(
        ...
        if isinstance(indices, Rotation):
            ...
            if axis is None:
                ...
                return
            new = Rotation(indices, axis, self.all_atoms[indices].positions)
        try:
...
class Internals(BaseInternals):
    def __init__(
        ...
        if cons is None:
            ...
            if (dummies is not None and dummies is not cons.dummies) or (
                dinds is not None and dinds is not cons.dinds
            ):
                raise RuntimeError(

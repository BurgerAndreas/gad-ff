...
def convert_deepmd_to_custom(deepmd_dir, output_path):
    subdirs = sorted(
        [
            d
            for d in os.listdir(deepmd_dir)
            if os.path.isdir(os.path.join(deepmd_dir, d))
        ]
    )
    ...
    for subdir in subdirs:
        ...
        output_file = os.path.join(
            raw_dir, f"{os.path.basename(output_path)}_{subdir}.pickle"
        )
        with open(output_file, "wb") as f:
            pickle.dump(
                {"E": E, "F": F, "R": R, "z": Z, "cell": C, "natoms": N, "stress": S}, f
            )

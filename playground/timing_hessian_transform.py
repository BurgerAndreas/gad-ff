...
from ocpmodels.hessian_graph_transform import (
    ...
    generate_fullyconnected_graph_nopbc,
)
...
def run_scaling_test(device):
    ...
    print(
        f"{'Batch Size':<10} {'Graph Gen':<10} {'Msg Idx':<10} {'Diag Idx':<10} {'Full Xform':<12} {'Per-Sample':<12}"
    )
    print("-" * 70)
    ...
    for i, batch_size in enumerate(batch_sizes):
        ...
        print(
            f"{batch_size:<10} {results['graph_gen_time'][i]:<10.4f} {results['message_idx_time'][i]:<10.4f} "
    ...
    if len(filtered_indices) > 1:
        ...
        x_data = np.array(
            [results["total_atoms"][i] for i in filtered_indices]
        ).reshape(-1, 1)

        # Analyze scaling for each timing component
        components = {
            "Graph Generation": [
                results["graph_gen_time"][i] for i in filtered_indices
            ],
            "Message Indices": [
                results["message_idx_time"][i] for i in filtered_indices
            ],
            "Diagonal Indices": [
                results["diagonal_idx_time"][i] for i in filtered_indices
            ],
            "Full Transform": [
                results["full_transform_time"][i] for i in filtered_indices
            ],
        }

        print(
            f"{'Component':<18} {'Slope (s/atom)':<15} {'Intercept (s)':<15} {'R²':<10}"
        )
        print("-" * 70)
        ...
        for comp_name, y_data in components.items():
            ...
            print(
                f"{comp_name:<18} {reg.coef_[0]:<15.6f} {reg.intercept_:<15.6f} {r_squared:<10.3f}"
            )
        ...
        per_atom_times = [
            results["full_transform_time"][i] / results["total_atoms"][i]

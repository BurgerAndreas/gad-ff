...
rdg1_csv_columns = {
    ...
    "DE_F": {"description": "Activation energy of the forward reaction", "unit": "kcal/mol"},
    "DE_B": {"description": "Activation energy of the backward reaction", "unit": "kcal/mol"},
    "DG_F": {"description": "Free energy of activation of the forward reaction", "unit": "kcal/mol"},
    "DG_B": {"description": "Free energy of activation of the backward reaction", "unit": "kcal/mol"},
    "DH": {"description": "Enthalpy of reaction (forward reaction)", "unit": "kcal/mol"}
}
...
if __name__ == "__main__":
    ...
    for Rind, Rxn in tqdm(rxns.items()):
        ...
        if len(rxn_row) == 1:
            AtomMapped_Rsmiles = rxn_row['reactant'].iloc[0]  # atom-mapped reactant smiles
            AtomMapped_Psmiles = rxn_row['product'].iloc[0]  # atom-mapped product smiles
        elif len(rxn_row) > 1:
    ...
    print(f"{len(missing_mols)} / {len(rxns)} reactions are missing -> {len(RP_dict)} reactions in total")

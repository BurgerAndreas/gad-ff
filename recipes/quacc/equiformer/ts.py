"""Transition state recipes for the Equiformer code."""
...
from recipes.quacc.equiformer.core import freq_job, relax_job
...
if has_geodesic_interpolate:
    ...
    print("geodesic-interpolate not found") 
...
def ts_job(
    ...
    **calc_kwargs
    ...
    opt_ts_summary = Summarize(additional_fields={"name": "Equiformer TS"}).opt(dyn)
    ...
    opt_ts_summary["trajectory"] = None
...
def irc_job(
    ...
    **calc_kwargs
    ...
    with change_settings({"CHECK_CONVERGENCE": False}):
        dyn = Runner(atoms, calc).run_opt(**opt_flags)
        opt_irc_summary = Summarize(
            additional_fields={"name": f"Equiformer IRC: {direction}"}
    ...
    opt_irc_summary["trajectory"] = None
...
def quasi_irc_job(
    ...
    run_freq
        Whether to run the frequency analysis
    irc_job_kwargs
        Keyword arguments to use for the irc_job
    relax_job_kwargs
        Keyword arguments to use for the relax_job
    freq_job_kwargs
        Keyword arguments to use for the freq_job
...
def neb_job(
    ...
    **calc_kwargs,
    ...
    interpolation_method
        ...
        Defaults to linear.
    relax_job_kwargs
        Keyword arguments to use for the relax_job function.
    interpolate_kwargs
    ...
    **calc_kwargs
    ...
    relax_summary_r["trajectory"] = None
    relax_summary_p["trajectory"] = None
...
def geodesic_job(
    ...
    **calc_kwargs,
    ...
    product_atoms
        The Atoms object representing the product structure.
    relax_job_kwargs
        Keyword arguments to use for relax_job function.
    ...
    **calc_kwargs
    ...
    # Define calculator
    reactant_atoms.calc = EquiformerASECalculator(**calc_flags)
    product_atoms.calc = EquiformerASECalculator(**calc_flags)
    ...
    potential_energies = []
    for image in images:
        image.calc = EquiformerASECalculator(**calc_flags)
...
def _get_hessian(atoms: Atoms, calc, method="autodiff", **calc_kwargs) -> NDArray:
    """
    ...
    configuration and uses the Equiformer machine learning calculator to
    calculate the Hessian matrix using autodiff. The calculated Hessian matrix is then
    returned.
    ...
    **calc_kwargs
    ...
    if method == "autodiff":
        ...
        hessian = calc.get_hessian_prediction(atoms)
    else:
        raise ValueError(f"Invalid method: {method}")
    return hessian.reshape((3 * len(atoms), 3 * len(atoms)))

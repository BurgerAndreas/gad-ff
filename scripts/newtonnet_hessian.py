...
parser.add_argument(
    ...
    default="configs/create_gad_dataset_composition.yaml",
)
parser.add_argument(
    "-p",
    "--parser",
...
if parser in ["md17", "ccsd"]:
    train_gen, val_gen, test_gen, tr_steps, val_steps, test_steps, normalizer = (
        parse_train_test(settings, device[0])
    )
    ...
    (
        ...
        normalizer,
        test_energy_hash,
    ) = parse_ani_data(settings, device[0])
    train_mode = "energy"
    print("data set:", "ANI")
    ...
    (
        ...
        normalizer,
        test_energy_hash,
    ) = parse_methane_data(settings, device[0])
    train_mode = "energy/force"
    ...
    train_gen, val_gen, test_gen, tr_steps, val_steps, test_steps, normalizer = (
        parse_t1x_data(settings, device[0])
    )
    train_mode = "energy/force"

...
csv_path = "results/wandb_export_loss_ablation.csv"
...
# Desired display order for loss ablation
_desired_order = ["MSE", "MAE", "MAE+Sub"]


def _loss_label(name: str) -> str:
    s = str(name).lower()
    if "luca" in s or "sub" in s:
        return "MAE+Sub"
    if "mse" in s:
        return "MSE"
    if "mae" in s:
...
# Minimal subset for calculations
df = df0[["Name", "hessian_method", "model_is_ts", "true_is_ts", "is_ts_agree"]].copy()

# Map names to human-friendly loss labels and enforce order
df["Name"] = df["Name"].apply(_loss_label)
df["Name"] = pd.Categorical(df["Name"], categories=_desired_order, ordered=True)

# Delta and optional renames
df["delta"] = df["model_is_ts"] - df["true_is_ts"]
# Keep method pretty name for any prints (not used in LaTeX here)
df["hessian_method"] = df["hessian_method"].map(hessian_method_to_name)

# Sort rows by desired loss order
...
lines.append("\\hline")
lines.append(
    "Name & TPR $\\uparrow$ & FPR $\\downarrow$ & FNR $\\downarrow$ & TNR $\\uparrow$ & Precision $\\uparrow$ & Accuracy $\\uparrow$ \\\\"
)
lines.append("\\hline")
...
for i, comp in enumerate(["TP", "FP", "FN", "TN"]):
    ...
    if left is None:
        left = vals * 0
    plt.barh(plot_df.index.astype(str), vals, left=left, label=comp)
    left = left + vals
plt.xlim(0, 1)

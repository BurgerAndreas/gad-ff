...
def test_dataset_consistency(original_dataset_name="ts1x-val.lmdb", eigen_dataset_name="ts1x-val-eigen.lmdb", num_samples=10):
    ...
    if len(original_dataset) != len(eigen_dataset):
        print(f"ERROR: Dataset sizes don't match! Original: {len(original_dataset)}, Eigen: {len(eigen_dataset)}")
        return
    
    print(f"\nComparing first {num_samples} samples...")
    
    # Expected new fields in eigen dataset
    eigen_fields = {'hessian_eigenvalue_1', 'hessian_eigenvalue_2', 'hessian_eigenvector_1', 'hessian_eigenvector_2'}
    ...
    for i in range(min(num_samples, len(original_dataset))):
        ...
        if missing_original_keys:
            print(f"  Sample {i}: ERROR - Missing original keys in eigen dataset: {missing_original_keys}")
            all_consistent = False
            continue
        
        if expected_new_keys != eigen_fields:
            print(f"  Sample {i}: ERROR - Unexpected new keys. Expected: {eigen_fields}, Got: {expected_new_keys}")
            all_consistent = False
            continue
        ...
        for key in original_keys:
            ...
            if isinstance(original_value, torch.Tensor):
                if not torch.equal(original_value, eigen_value):
                    ...
                    print(f"    Original shape: {original_value.shape}, Eigen shape: {eigen_value.shape}")
        ...
        try:
            ...
            if eigenval_1.shape != torch.Size([1]):
                print(f"  Sample {i}: ERROR - hessian_eigenvalue_1 wrong shape: {eigenval_1.shape}, expected [1]")
                sample_consistent = False
                all_consistent = False
            
            if eigenval_2.shape != torch.Size([1]):
                print(f"  Sample {i}: ERROR - hessian_eigenvalue_2 wrong shape: {eigenval_2.shape}, expected [1]")
                sample_consistent = False
                all_consistent = False
            ...
            if eigenvec_1.shape != expected_eigenvec_shape:
                print(f"  Sample {i}: ERROR - hessian_eigenvector_1 wrong shape: {eigenvec_1.shape}, expected {expected_eigenvec_shape}")
                sample_consistent = False
                all_consistent = False
            
            if eigenvec_2.shape != expected_eigenvec_shape:
                print(f"  Sample {i}: ERROR - hessian_eigenvector_2 wrong shape: {eigenvec_2.shape}, expected {expected_eigenvec_shape}")
                sample_consistent = False
                all_consistent = False
...
def test_eigen_dataset(dataset_name="ts1x-val-eigen.lmdb", max_batches=-1):
    ...
    # ---- Load eigen dataset ----
    try:
        eigen_dataset = LmdbDataset(eigen_dataset_path, transform=fix_hessian_eigen_transform)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Testing batches")):
            ...
            stored_eigenvecs = batch.hessian_eigenvector_1.cpu()  # Shape: [total_atoms, 3]
            ...
            for sample_idx in range(batch.batch.max().item() + 1):
                ...
                first_eigenvector = stored_eigenvecs[atom_idx:atom_idx + n_atoms].flatten()  # [natoms*3]

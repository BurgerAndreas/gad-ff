...
def process_subset(dataset_file, start_idx, end_idx, job_id, save_hessian=False):
    ...
    if not successfully_removed:
        raise RuntimeError(f"Temp output database file {temp_output_path} already exists!")
    
    print(f"Processing subset {start_idx}:{end_idx} of {input_lmdb_path} -> {temp_output_path}")
    ...
    print(f"Processing subset with {len(subset_dataset)} samples (indices {start_idx}:{end_idx})")
    ...
    print(f"Processed {len(processed_samples)} samples with indices: {min(processed_samples)}-{max(processed_samples)}")
...
def combine_subsets(dataset_file, save_hessian=False):
    ...
    with out_env.begin(write=True) as out_txn:
        for temp_file in tqdm(sorted(temp_files)):
            ...
            with temp_env.begin() as temp_txn:
                ...
                print(f"Processing temp file with job_id={metadata['job_id']}, "
    ...
    # Verify completeness
    expected_indices = set(range(min(all_processed_indices), max(all_processed_indices) + 1))
    missing_indices = expected_indices - all_processed_indices
    if missing_indices:
        print(f"WARNING: Missing indices: {sorted(list(missing_indices))[:10]}{'...' if len(missing_indices) > 10 else ''}")
...
def main():
    parser = argparse.ArgumentParser(description="Parallel Hessian eigenvalue processing")
    parser.add_argument("--process", action="store_true", help="Process a subset of the dataset")
    parser.add_argument("--combine", action="store_true", help="Combine processed subsets")
    parser.add_argument("--dataset", type=str, required=True, help="Dataset filename (e.g., RGD1.lmdb)")
    parser.add_argument("--start-idx", type=int, help="Starting index for subset processing")
    parser.add_argument("--end-idx", type=int, help="Ending index for subset processing")
    parser.add_argument("--job-id", type=int, help="Job ID for unique temporary file naming")
    parser.add_argument("--save-hessian", action="store_true", help="Save original Hessian data")
    parser.add_argument("--samples-per-job", type=int, default=10000, help="Samples per job for script generation")

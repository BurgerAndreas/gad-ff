...
def main() -> None:
    ...
    predict_rows = [
        r
        for r in select_by_method(rows, "predict")
        if (r.get("model_name") or "") == "EquiformerV2"
    ]
    ...
    lam1e_best = _best_index(
        [m["lambda1_eckart"] for m in metrics_list], maximize=False
    )
    lam2e_best = _best_index(
        [m["lambda2_eckart"] for m in metrics_list], maximize=False
    )
    ...
    lines.append(
        " & eV/\\AA$^2$ & eV/\\AA$^2$ & unitless & eV/\\AA$^2$ & eV/\\AA$^2$ & eV/\\AA$^2$ & eV/\\AA$^2$ & unitless \\\\"
    )
    lines.append("\\hline")
    ...
    for idx, m in enumerate(metrics_list):
        ...
        l1e_str = _bold(
            _fmt(m["lambda1_eckart"]), (lam1e_best is not None and idx == lam1e_best)
        )
        l2e_str = _bold(
            _fmt(m["lambda2_eckart"]), (lam2e_best is not None and idx == lam2e_best)
        )
        ce_str = _bold(
            _fmt(m["cos1_eckart"]), (cose_best is not None and idx == cose_best)
        )
        t_str = _bold(
            _fmt_time_ms(m["time_ms"]), (t_best is not None and idx == t_best)
        )
        # & {m['model']}
        lines.append(

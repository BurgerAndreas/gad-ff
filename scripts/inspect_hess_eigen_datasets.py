...
def compute_statistics(eigenvals_1, eigenvals_2, dataset_name):
    ...
    is_index1_saddle = ((eigenvals_1 < 0) & (eigenvals_2 > 0)) | ((eigenvals_1 > 0) & (eigenvals_2 < 0))
...
def plot_logx_histograms(eigenvals_1, eigenvals_2, dataset_name, output_dir):
    ...
    if len(ev1_pos) > 0:
        ax1.hist(np.log10(ev1_pos), bins=30, alpha=0.7, label='Positive (log10)', color='blue')
    if len(ev1_neg) > 0:
        ax1.hist(np.log10(-ev1_neg), bins=30, alpha=0.7, label='Negative (log10 of |val|)', color='red')
    ...
    if len(ev2_pos) > 0:
        ax2.hist(np.log10(ev2_pos), bins=30, alpha=0.7, label='Positive (log10)', color='blue')
    if len(ev2_neg) > 0:
        ax2.hist(np.log10(-ev2_neg), bins=30, alpha=0.7, label='Negative (log10 of |val|)', color='red')
...
def plot_kde_joint_distribution(eigenvals_1, eigenvals_2, dataset_name, output_dir):
    ...
    try:
        ...
        sns.kdeplot(x=eigenvals_1, y=eigenvals_2, ax=ax, fill=True, cmap='Blues', alpha=0.7)
        sns.kdeplot(x=eigenvals_1, y=eigenvals_2, ax=ax, levels=5, colors='black', alpha=0.5, linewidths=1)
    except Exception as e:
...
def create_eigenvalue_plots(eigenvals_1, eigenvals_2, dataset_name, output_dir="eigenvalue_plots"):
    """
...
def print_statistics_table(all_stats):
    ...
    for stats in all_stats:
        ...
        print(f"  Eigenvalue 1: mean={stats['ev1_mean']:.4f}, std={stats['ev1_std']:.4f}, range=[{stats['ev1_min']:.4f}, {stats['ev1_max']:.4f}]")
        print(f"  Eigenvalue 2: mean={stats['ev2_mean']:.4f}, std={stats['ev2_std']:.4f}, range=[{stats['ev2_min']:.4f}, {stats['ev2_max']:.4f}]")
        print(f"  Sign distribution:")
        print(f"    EV1: {stats['ev1_positive']} pos, {stats['ev1_negative']} neg, {stats['ev1_zero']} zero")
        print(f"    EV2: {stats['ev2_positive']} pos, {stats['ev2_negative']} neg, {stats['ev2_zero']} zero")
        print(f"  Critical point types:")
        print(f"    Index-1 saddles: {stats['index1_saddle_count']} ({stats['index1_saddle_fraction']*100:.2f}%)")
        print(f"    Local minima (both pos): {stats['both_positive_count']} ({stats['both_positive_fraction']*100:.2f}%)")
        print(f"    Local maxima (both neg): {stats['both_negative_count']} ({stats['both_negative_fraction']*100:.2f}%)")

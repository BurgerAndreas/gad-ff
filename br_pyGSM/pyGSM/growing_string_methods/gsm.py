...
class GSM(object):
    ...
    def __init__(
        ...
        self.n0 = (
            1  # something to do with added nodes? "first node along current block"
        )
        ...
        self.TS_E_0 = None
        self.dE_iter = 100.0  # change in max TS node
    ...
    def TSnode(self):
        ...
        if (
            ...
            for i, node in enumerate(self.nodes):
                if node is not None:
                    energies[i] = (node.PES.PES1.energy + node.PES.PES2.energy) / 2.0
    ...
    def add_node(nodeR, nodeP, stepsize, node_id, **kwargs):
        ...
        if nodeP is None:
            ...
            new_node = Molecule.copy_from_options(
                MoleculeA=nodeR, xyz=new_xyz, new_node_id=node_id
            )
    ...
    def interpolate(start_node, end_node, num_interp):
        ...
        for n in range(num_interp):
            if num_nodes - nn > 1:
                stepsize = 1.0 / float(num_nodes - nn)
            else:
                stepsize = 0.5
            if sign == 1:
                iR = nR - 1
                ...
                # nifty.logger.debug(" Energy of node {} is {:5.4}".format(nR,nodes[nR].energy-E0))
                nR += 1
                nn += 1
            ...
            for i in driving_coords:
                if 'ADD' in i:
                    ...
                    if len(i) == 3:
                        ...
                        d0 = (
                            atoms[index[0]].vdw_radius + atoms[index[1]].vdw_radius
                        ) / 2.8
                    elif len(i) == 4:
                        d0 = i[3]
                    ...
                    # nifty.logger.debug(" angle: %s is index %i " %(angle,ang_idx))
                    if print_level > 0:
                        nifty.logger.debug(
                            (
                    ...
                    if tor_diff > 180.0:
                        ...
                        tor_diff += 360.0
                    ictan[prim_idx] = -tor_diff * np.pi / 180.0

                    if tor_diff * np.pi / 180.0 > 0.1 or tor_diff * np.pi / 180.0 < 0.1:
                        bdist += np.dot(ictan[prim_idx], ictan[prim_idx])
                    if print_level > 0:
                        nifty.logger.debug(
                            (
                    ...
                    if oop_diff > 180.0:
                        ...
                        oop_diff += 360.0
                    ictan[prim_idx] = -oop_diff * np.pi / 180.0

                    if oop_diff * np.pi / 180.0 > 0.1 or oop_diff * np.pi / 180.0 < 0.1:
                        bdist += np.dot(ictan[prim_idx], ictan[prim_idx])
                    if print_level > 0:
                        nifty.logger.debug(
                            (
    ...
    def get_three_way_tangents(nodes, energies, find=True, n0=0, print_level=0):
        ...
        TSnode = np.argmax(
            energies
        )  # allow for the possibility of TS node to be endpoints?
    ...
    def ic_reparam(
        ...
        if climbing:
            for n in range(1, TSnode):
                ideal_progress_gained[n] = 1.0 / (TSnode)
            for n in range(TSnode + 1, nnodes):
                ideal_progress_gained[n] = 1.0 / (nnodes - TSnode - 1)
            ideal_progress_gained[TSnode] = 0.0
        else:
            for n in range(1, nnodes):
            ...
            if climbing:
                ...
                if NUM_CORE > 1:
                    ...
                    for n, node in enumerate(
                        nodes[1:TSnode] + nodes[TSnode + 1 : nnodes - 1]
                    ):
    ...
    #        #nifty.logger.debug(' Rotating reference axis to current axis')
    #        I = np.eye(3)
    #        v = np.cross(ref_axis,axis)
    #        if v.all()==0.:
    #            nifty.logger.debug('Rotation is identity')
    #            R=I
    #        else:
    ...
    #        #nifty.logger.debug(' overlap of ref-axis and axis (should be 1.) %1.2f' % np.dot(new_ref_axis,axis))

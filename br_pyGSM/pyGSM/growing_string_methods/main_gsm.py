...
class MainGSM(GSM):
        ...
        while oi < max_iter:
            ...
            if not self.find:
                ts_cgradq = np.linalg.norm(
                    np.dot(
                        self.nodes[self.TSnode].gradient.T,
            ...
            # => set stage <= #
            stage_changed = self.set_stage(
                totalgrad, sum_gradrms, ts_cgradq, ts_gradrms, fp
            )
            ...
            if not stage_changed:
                ...
                if self.find and (not self.optimizer[self.TSnode].maxol_good or added):
                    ...
                    self.find
                    and (
                        self.optimizer[self.TSnode].nneg > 3
                        or self.optimizer[self.TSnode].nneg == 0
                        or self.hess_counter > 10
                        or np.abs(self.TS_E_0 - self.emax) > 10.0
                    )
            ...
            if self.mp_cores == 1:
                for n in range(1, self.nnodes - 1):
                    if self.nodes[n] is not None:
                        Vecs = self.newic.coord_obj.build_dlc(
                            self.nodes[n].xyz, self.ictan[n]
                        )
            ...
            if self.find or self.climb:
                ...
                if self.mp_cores == 1:
                    ...
                    for i, n in enumerate(
                        chain(range(1, TSnode), range(TSnode + 1, self.nnodes - 1))
                    ):
                        self.nodes[n].coord_basis = Vecs[i]

                    if update_TS:
                        Vec = self.newic.coord_obj.build_dlc(
                            self.nodes[TSnode].xyz, self.ictan[TSnode]
                        )
                ...
                if self.mp_cores == 1:
                    ...
                    for n in range(1, self.nnodes - 1):
                        Vecs.append(
                            self.newic.coord_obj.build_dlc(
                                self.nodes[n].xyz, self.ictan[n]
                            )
                        )
    ...
    def set_stage(self, totalgrad, sumgradrms, ts_cgradq, ts_gradrms, fp):
        ...
        all_converged_climb = all(
            [
                self.nodes[n].gradrms < self.optimizer[n].conv_grms * 2.5
                for n in range(1, self.nnodes - 1)
            ]
        )
        stage_changed = False
        ...
        if (
            ...
            # (totalgrad < 0.2 and ts_gradrms < self.CONV_TOL*20. and ts_cgradq < 0.02) or  #
                ...
                and (
                    (
                        totalgrad < 0.3 and ts_gradrms < self.CONV_TOL * 20.0
                    )  # I hate totalgrad  and ts_cgradq < 0.02
                    or (all_converged)
                    or (
                        ts_gradrms < self.CONV_TOL * 2.5 and ts_cgradq < 0.01
                    )  # used to be 5
                )
    ...
    def add_GSM_nodeR(self, newnodes=1):
        ...
        for i in range(newnodes):
            ...
            # nifty.logger.debug("%i %i %i" %(iR,iP,iN))

            # nifty.logger.debug(" Aligning")
    ...
    def add_GSM_nodeP(self, newnodes=1):
        ...
        for i in range(newnodes):
            ...
            # align center of mass  and rotation
            # nifty.logger.debug("%i %i %i" %(n1,n3,n2))
            # nifty.logger.debug(" Aligning")
            # self.nodes[-self.nP].xyz = self.com_rotate_move(n1,n3,n2)
            # nifty.logger.debug(" getting energy for node %d: %5.4f" %(self.nnodes-self.nP,self.nodes[-self.nP].energy - self.nodes[0].V0))
        return
    ...
    def reparameterize(self, ic_reparam_steps=8, n0=0, nconstraints=1):
        ...
        if self.interp_method == 'DLC':
            ...
            self.ic_reparam(
                ...
                climbing=(self.climb or self.find),
                ic_reparam_steps=ic_reparam_steps,
                NUM_CORE=self.mp_cores,
            )
        return

    def ic_reparam_g(
        self, ic_reparam_steps=4, n0=0, reparam_interior=True
    ):  # see line 3863 of gstring.cpp
        """
        ...
        for i in range(ic_reparam_steps):
            self.ictan, self.dqmaga = self.get_tangents_growing()
            totaldqmag = np.sum(self.dqmaga[n0 : self.nR - 1]) + np.sum(
                self.dqmaga[self.nnodes - self.nP + 1 : self.nnodes]
            )
            ...
            if self.mp_cores > 1:
                ...
                Vecs = pool.map(
                    ...
                    (
                        ...
                        for n, ntan in zip(move_list, tan_list)
                        if rpmove[n] < 0
                    ),
                )
                pool.close()
    ...
    def modify_TS_Hess(self):
        ...
        nifty.logger.debug(" TS Hess init'd w/ existing Hintp")
    ...
    def mult_steps(self, n, opt_steps):
        ...
        if (
            (self.find or self.climb)
            and self.energies[n] > self.energies[self.TSnode] * 0.9
            and n != tsnode
        ):  #
            exsteps = 2
    ...
    def com_rotate_move(self, iR, iP, iN):
        ...
        if self.nnodes - self.current_nnodes + 1 != 1:
            mfrac = 1.0 / (self.nnodes - self.current_nnodes + 1)
        ...
        # nifty.logger.debug("aligning to com")
        # nifty.logger.debug(dist)
        # xyz1 += dist
    ...
    def find_peaks(self, rtype='opting'):
        ...
        if energies[nnodes - 1] > 15.0:
            if nnodes - 3 > 0:
                if (
                    (energies[nnodes - 1] - energies[nnodes - 2]) < alluptol2
    ...
    def is_converged(self, totalgrad, fp, rtype, ts_cgradq):
        ...
        # nifty.logger.debug(" Number of imaginary frequencies %i" % self.optimizer[self.TSnode].nneg)
    ...
    def setup_from_geometries(
        self,
        input_geoms,
        reparametrize=True,
        restart_energies=True,
        ...
        self.gradrms = [0.0] * nstructs
        self.dE = [1000.0] * nstructs
        ...
        for struct in range(1, nstructs - 1):
            self.nodes[struct] = Molecule.copy_from_options(
                self.nodes[struct - 1],
    ...
    def add_node_before_TS(self):
        ...
        new_optimizers[self.TSnode] = self.optimizer[0].__class__(
            self.optimizer[0].options.copy()
        )

        for n in range(self.TSnode + 1, self.nnodes + 1):
            new_node_list[n] = Molecule.copy_from_options(
                MoleculeA=self.nodes[n - 1], new_node_id=n
            )
    ...
    def add_node_after_TS(self):
        ...
        new_optimizers[self.TSnode + 1] = self.optimizer[0].__class__(
            self.optimizer[0].options.copy()
        )
    ...
    def set_node_convergence(self):
        ...
        factor = 5.0 if (self.climber or self.finder) else 1.0
        TSnode = self.TSnode
        for n in range(1, self.nnodes - 1):
            if self.nodes[n] is not None:
                ...
                if (
                    (self.climb or self.find)
                    and self.energies[n] > self.energies[TSnode] * 0.75
                    and n != TSnode
                ):

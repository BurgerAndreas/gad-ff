...
class SE_GSM(MainGSM):
    ...
    def ic_reparam_g(
        self, ic_reparam_steps=4, n0=0, nconstraints=1
    ...
    def is_converged(self, totalgrad, fp, rtype, ts_cgradq):
        if (
            ...
            if self.nodes[self.nR - 1].gradrms > self.options['CONV_TOL']:
                ...
                self.add_last_node(1)
                self.nnodes = self.nR
                self.active[self.nnodes - 1] = (
                    False  # GSM makes self.active[self.nnodes-1]=True as well
                )
    ...
    def check_for_reaction_g(self, rtype, driving_coords):
        ...
        #    nifty.logger.debug(" reducing TS convergence because nneg>1")

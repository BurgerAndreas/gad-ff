...
class PES(object):
    ...
    def create_2dgrid(
        self, xyz, xvec, yvec, nx, ny, xmin=-1.0, ymin=-1.0, xmax=1.0, ymax=1.0
    ):
        xyz = xyz.flatten()
    ...
    def get_energy(self, xyz):
        ...
        if self.FORCE is not None:
            for i in self.FORCE:
                ...
                # nifty.logger.debug(" Force energy: {} kcal/mol".format(fdE))
        kdE = 0.0
        ...
        return (
            self.lot.get_energy(xyz, self.multiplicity, self.ad_idx) + fdE + kdE
        )  # Kcal/mol
    ...
    def normal_modes(
        ...
        #    nifty.logger.debug('%10.6E %10.6E %11.3E' % (wval, wval2, np.abs(wval - wval2)))

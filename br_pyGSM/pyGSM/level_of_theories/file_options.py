...
class File_Options(object):
    ...
    def set_active(
        self, key, default, typ, doc, allowed=None, depend=True, clash=False, msg=None
    ):
        ...
        if type(allowed) is list:
            ...
            if val not in allowed:
                raise Exception(
                    ...
                    % (key, str(val), str(allowed))
                )
        if typ is bool and type(val) == int:
            val = bool(val)
        if val is not None and type(val) is not typ:
            raise Exception(
                "Tried to set option \x1b[1;91m%s\x1b[0m to \x1b[94m%s\x1b[0m but it's not the right type (%s required)"
                % (key, str(val), str(typ))
            )
...
if __name__ == "__main__":
    ...
    nifty.logger.debug(id(fo))
    nifty.logger.debug(id(fo2))

    nifty.logger.debug(fo)
    ...
    nifty.logger.debug(tmp2.crystal)

...
class TeraChemCloud(Lot):
    ...
    def __init__(self, options):
        ...
        if self.lot_inp_file is not None:
            ...
            nifty.logger.debug(' done executing lot_inp_file')
    ...
    def run(self, coords):
        ...
        for state in self.states:
            # nifty.logger.debug("on state %d" % state[1])
            ...
            if self.orbfile:
                ...
                nifty.logger.debug(' orbfile is %s' % self.orbfile)
            else:
                nifty.logger.debug(' generating orbs from guess')
            job_id = self.TC.submit(coords, grad_options)
            results = self.TC.poll_for_results(job_id)
            while results['message'] == 'job not finished':
                results = self.TC.poll_for_results(job_id)
                nifty.logger.debug(results['message'])
                nifty.logger.debug('sleeping for 1')
                time.sleep(1)
                sys.stdout.flush()

            # nifty.logger.debug((json.dumps(results, indent=2, sort_keys=True)))

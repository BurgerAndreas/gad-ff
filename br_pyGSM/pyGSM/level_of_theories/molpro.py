...
class Molpro(Lot):
    ...
    def runall(self, geom, runtype=None):
        ...
        output = subprocess.Popen(
            command,
            stdout=open("scratch/{}/gopro.out".format(self.node_id), "w"),
            stderr=subprocess.PIPE,
        ).communicate()
        nifty.logger.debug(output[0])
    ...
    def parse(self):
        ...
        with open(tempfileout, "r") as f:
            for line in f:
                if line.startswith(
                    "GRADIENT FOR STATE", 7
                ):  # will work for SA-MC and RSPT2 HF
                    for i in range(3):
                        next(f)
                    for i in range(len(geom)):
                        findline = next(f, "").strip()
                        mobj = re.match(
                            r"^\s*(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s*$", findline
                        )
                        tmpgrad.append(
                            [
                                float(mobj.group(2)),
    ...
    def copy(cls, lot, options, copy_wavefunction=True):
        ...
        # nifty.logger.debug(" creating copy, new node id =",node_id)
        # nifty.logger.debug(" old node id = ",self.node_id)
        node_id = options.get("node_id", 1)
        if node_id != lot.node_id and copy_wavefunction:
            cmd = "cp scratch/mp_{:04d}_{:04d} scratch/mp_{:04d}_{:04d}".format(
                lot.ID, lot.node_id, lot.ID, node_id
            )
            nifty.logger.debug(cmd)
            os.system(cmd)
...
if __name__ == "__main__":
    ...
    molpro = Molpro.from_options(
        states=[(1, 0), (1, 1)],
        fnm=filepath,
        lot_inp_file="../../data/ethylene_molpro.com",
        coupling_states=(0, 1),
    )
    geom = manage_xyz.read_xyz(filepath)
    xyz = manage_xyz.xyz_to_np(geom)
    nifty.logger.debug(molpro.get_energy(xyz, 1, 0))
    nifty.logger.debug(molpro.get_gradient(xyz, 1, 0))
    nifty.logger.debug(molpro.get_coupling(xyz, 1, 0, 1))

...
class TeraChem(Lot):
    def __init__(self, options):
        ...
        self.file_options.set_active(
            ...
            int,
            '',
            depend=(self.file_options.casscf == 'yes'),
            msg='',
        )
        self.file_options.set_active(
            'alphacas',
        ...
        self.file_options.set_active(
            ...
            float,
            '',
            depend=(self.file_options.alphacas == 'yes'),
            msg='',
        )
        self.file_options.set_active(
            'casscfmacroiter',
        ...
        self.file_options.set_active(
            ...
            '',
            depend=(
                self.file_options.casscf == 'yes' or self.file_options.casci == 'yes'
            ),
            msg='',
        )
        self.file_options.set_active(
        ...
        # hh-tda
        self.file_options.set_active(
            'hhtda', 'no', str, '', clash=(self.file_options.casscf == 'yes'), msg=''
        )
        self.file_options.set_active(
            'hhtdasinglets',
            3,
            int,
            '',
            depend=(self.file_options.hhtda == 'yes'),
            msg='',
        )
        self.file_options.set_active(
            'hhtdatriplets',
        ...
        self.file_options.set_active(
            'cphfiter', None, int, '', depend=(self.file_options.hhtda == 'yes'), msg=''
        )  # a good value could be 1000
        self.file_options.set_active(
            'cphfalgorithm',
            None,
            str,
        ...
        self.file_options.set_active(
            ...
            int,
            '',
            depend=(self.file_options.sphericalbasis == 'yes'),
            msg='',
        )
        ...
        self.file_options.set_active(
            'fon_method', None, str, '', depend=(self.file_options.fon == 'yes'), msg=''
        )
        self.file_options.set_active(
            'fon_temperature',
            0.3,
            float,
            '',
            depend=(self.file_options.fon == 'yes'),
            msg='',
        )
        ...
        self.file_options.set_active(
            ...
            float,
            '',
            depend=(self.file_options.fomo == 'yes'),
            msg='',
        )
        self.file_options.set_active(
            'fomo_nocc',
            self.file_options.closed,
            int,
            '',
            depend=(self.file_options.fomo == 'yes'),
            msg='',
        ...
        self.file_options.set_active(
            ...
            '',
            clash=(
                self.file_options.casscf == 'yes' or self.file_options.fomo == 'yes'
            ),
        ...
        self.file_options.set_active(
            ...
            str,
            '',
            depend=(self.file_options.cis == 'yes' or self.file_options.hhtda == 'yes'),
        )
        self.file_options.set_active(
            'cisnumstates',
            4,
        ...
        self.file_options.set_active(
            'cisguessvecs',
            self.file_options.cisnumstates,
            int,
            '',
            depend=(self.file_options.cis == 'yes' or self.file_options.hhtda == 'yes'),
        )
        ...
        self.file_options.set_active(
            'coordinates',
            'scratch/{:03}/{}/tmp.xyz'.format(self.ID, self.node_id),
            str,
            doc='',
        )
        self.file_options.force_active(
        ...
        self.file_options.set_active(
            'epsilon', 1.0, float, '', depend=self.file_options.pcm == 'yes'
        )
        ...
        if self.file_options.prmtop is None:
            ...
            self.file_options.force_active(
                'coordinates',
                'scratch/{:03}/{}/tmp.inpcrd'.format(self.ID, self.node_id),
            )
        ...
        self.file_options.set_active(
            'casguess',
            'scratch/{:03}/{}/c0.casscf'.format(self.ID, self.node_id),
            str,
            doc='guess for casscf',
            depend=(self.file_options.casscf == 'yes'),
            msg='',
        )
        ...
        if os.path.isfile(guess_file):
            if self.unrestricted:
                ...
                self.file_options.set_active(
                    'guess',
                    guess_file + guess_file2,
                    str,
                    doc='guess for dft/HF',
                    clash=(self.file_options.casscf or self.file_options.fomo),
                    depend=(os.path.isfile(guess_file)),
                    msg='guess does not exist deactivating for now',
                )
            else:
                self.file_options.set_active(
                    'guess',
            ...
            if 'casscf' in lot.file_options.ActiveOptions:
                ...
                if lot.unrestricted:
                    ...
                    if os.path.isfile(old_path):
                        ...
                        nifty.logger.debug(' no file to copy')
    ...
    def write_input(self, inpfilename, geom, mult, ad_idx, runtype='gradient'):
        ...
        if (
            'casscf' in self.file_options.ActiveOptions
            or 'casci' in self.file_options.ActiveOptions
        ):
            if runtype == 'gradient':
            ...
            'cis' in self.file_options.ActiveOptions
            or 'hhtda' in self.file_options.ActiveOptions
        ):
            if runtype == 'gradient':
    ...
    def run(self, geom, mult, ad_idx, runtype='gradient'):
        ...
        if (
            'guess' not in self.file_options.ActiveOptions
            and 'casscf' not in self.file_options.ActiveOptions
            or self.file_options.guess in ['sad', 'generate']
        ):
            if self.unrestricted:
                self.file_options.force_active(
                    'guess',
                    'scratch/{:03}/{}/ca0 scratch/{:03}/{}/cb0'.format(
    ...
    def runall(self, geom, runtype=None):
        ...
        if not self.gradient_states and not self.coupling_states or runtype == 'energy':
            nifty.logger.debug(' only calculating energies')
...
if __name__ == '__main__':
    ...
    nifty.logger.debug('getting energy')
    nifty.logger.debug(TC.get_energy(xyz, 1, 0))
    nifty.logger.debug(TC.get_energy(xyz, 1, 1))
    nifty.logger.debug('getting grad')
    nifty.logger.debug(TC.get_gradient(xyz, 1, 0))
    nifty.logger.debug(TC.get_gradient(xyz, 1, 1))

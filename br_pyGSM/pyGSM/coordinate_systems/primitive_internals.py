...
class PrimitiveInternalCoordinates(InternalCoordinates):
    ...
    def __init__(self, options):
        ...
        # nifty.logger.debug("done reordering %.3f" % time_reorder)
                    ...
                    if a < c:
                        ...
                        if np.abs(np.cos(Ang.value(coords))) < LinThre:
                            ...
                            if nnc == 0:
                                ...
                                # nifty.logger.debug("warning, adding translation, did you mean this?")
    ...
    def wilsonB(self, xyz):
        ...
        if xhash in self.stored_wilsonB:
            # nifty.logger.debug(" returning stored")
        ...
        # nifty.logger.debug(block_matrix.full_matrix(ans))
        # nifty.logger.debug("total B shape ",ans.shape)
        # nifty.logger.debug(" num blocks ",ans.num_blocks)
        # for block in ans.matlist:
        #    nifty.logger.debug(block)
    ...
    def GMatrix(self, xyz):
        ...
        t1 = time.time()
        # nifty.logger.debug(" done getting Bmat {}".format(t1-t0))
    ...
    def GInverse_EIG(self, xyz):
        ...
        # nifty.logger.debug("G-time: %.3f Inv-time: %.3f" % (time_G, time_inv))
            ...
            if info[-1] == "reg":
                ...
                if connect:
                    ...
                    if addcart:
                        ...
                        nodes = frag.nodes()
                        # nifty.logger.debug(" Nodes")
                        # nifty.logger.debug(nodes)
                        if len(nodes) >= 2:
                            self.tmp_add(
                                TranslationX(nodes, w=np.ones(len(nodes)) / len(nodes))
                            )
                            self.tmp_add(
                                TranslationY(nodes, w=np.ones(len(nodes)) / len(nodes))
                            )
                            self.tmp_add(
                                TranslationZ(nodes, w=np.ones(len(nodes)) / len(nodes))
                            )
                            ...
                            if a < c:
                                ...
                                if np.abs(np.cos(Ang.value(coords))) < LinThre:
                                    ...
                                    if nnc == 0:
                                        ...
                                        # Dot products of this vector with the Cartesian axes
                                        dots = [
                                            np.abs(np.dot(ei, nac)) for ei in np.eye(3)
                                        ]
                                        # Functions for adding Cartesian coordinate
                                        # carts = [CartesianX, CartesianY, CartesianZ]
                                        # nifty.logger.debug("warning, adding translation, did you mean this?")
    ...
    def reorderPrimsByFrag(self):
        ...
        # nifty.logger.debug(" Getting the block information")
        ...
        # nifty.logger.debug(" block info")
        # nifty.logger.debug(self.block_info)
        # nifty.logger.debug(" Done Ordering prims by block")
        # nifty.logger.debug("num blocks ",len(self.block_info))
        # if len(newPrims) != len(self.Internals):
        #    #nifty.logger.debug(np.setdiff1d(self.Internals,newPrims))
        #    raise RuntimeError("Not all internal coordinates have been accounted for. You may need to add something to reorderPrimitives()")
    ...
    def calcCg(self, xyz, gradx):
        ...
        for info in self.block_info:
            ...
            for p, prim in enumerate(self.Internals[sp:ep]):
                sder = np.reshape(
                    prim.second_derivative(xyz[sa:ea, :], start_idx=sa),
                    (3 * na, 3 * na),
                )
    ...
    def get_hybrid_indices(self, xyz):
        ...
        # nifty.logger.debug("prim start stop")
        # nifty.logger.debug(prim_idx_start_stop)

        prim_idx = []
        for info in prim_idx_start_stop:
            prim_idx += list(range(int(info[0]), int(info[1] + 1)))
        # nifty.logger.debug('prim indices')
        # nifty.logger.debug(prim_idx)

        # nifty.logger.debug(natoms)
        new_hybrid_indices = list(range(int(natoms)))
        # nifty.logger.debug(new_hybrid_indices)
        # for count,i in enumerate(prim_idx):
        #    nifty.logger.debug(i,end=' ')
        #    if (count+1) %20==0:
        #        nifty.logger.debug('')
        # nifty.logger.debug("")
        # nifty.logger.debug(type(new_hybrid_indices[0]))
        for elem in prim_idx:
            try:
                new_hybrid_indices.remove(elem)
            except:
                nifty.logger.debug(elem)
                nifty.logger.debug(type(elem))
                raise RuntimeError
        # nifty.logger.debug('hybrid indices')
        # nifty.logger.debug(new_hybrid_indices)
        ...
        # nifty.logger.debug(" hybrid start stop")
        # nifty.logger.debug(self.hybrid_idx_start_stop)
    ...
    def append_prim_to_block(self, prim, count=None):
            ...
            for info in self.block_info:
                if info[3] - info[2] != 3:  # this is a hybrid block skipping
                    if all([atom in range(info[0], info[1]) for atom in prim.atoms]):
                        break
                count += 1
            # nifty.logger.debug(" the prim lives in block {}".format(count))
        ...
        # nifty.logger.debug(" prims after inserting at elem {}".format(elem))
        # nifty.logger.debug(self.Internals)
        ...
        # nifty.logger.debug(new_block_info)
    ...
    def add_union_primitives(self, other):
        ...
        #        #nifty.logger.debug("this prim is in p2 but not p1",i)
        #        nifty.logger.debug("Adding prim {} that is in Other to Internals".format(i))
...
if __name__ == "__main__" and __package__ is None:
    ...
    if test_prims:
        ...
        nifty.logger.debug(xyz[0, :])
        nifty.logger.debug(xyz[0, :].shape)
        ...
        nifty.logger.debug(xyz[10:16, :].shape)
        ...
        nifty.logger.debug(der1)
        nifty.logger.debug(der1.shape)
    ...
    # nifty.logger.debug("Does p1 equal p2? ", p1==p2)

    # nifty.logger.debug(" Adding Angle 7-6-11 to p1")
    # angle = Angle(6,5,10)
    # nifty.logger.debug(angle)
    # p1.append_prim_to_block(angle)

    # nifty.logger.debug(p.calculate(xyz))
    # nifty.logger.debug(len(p.Internals))

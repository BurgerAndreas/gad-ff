...
class InternalCoordinates(object):
    ...
    def GMatrix(self, xyz, u=None):
        ...
        # t10 = t1-t0
        # t21 = t2-t1
        # nifty.logger.debug("time to form B-matrix %.3f" % t10)
        # nifty.logger.debug("time to mat-mult B %.3f" % t21)
        return BuBt
    ...
    def newCartesian(self, xyz, dQ, frozen_atoms=None, verbose=True):
        ...
        while True:
            ...
            dxyz = damp * block_matrix.dot(
                block_matrix.transpose(Bmat), block_matrix.dot(Ginv, dQ1)
            )

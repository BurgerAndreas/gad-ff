...
class DelocalizedInternalCoordinates(InternalCoordinates):
    def __init__(self, options):
        ...
        self.build_dlc(xyz)
        # nifty.logger.debug("vecs after build")
        # nifty.logger.debug(self.Vecs)
    ...
    #    #nifty.logger.debug("time to do block mult %.3f" % nifty.click())
    #    #Gq = np.dot(np.multiply(np.diag(Ginv)[:,None],Bmat),gradx)
    #    #nifty.logger.debug("time to do efficient mult %.3f" % nifty.click())
    #    return Gq
    ...
    def build_dlc(self, xyz, C=None):
        ...
        nifty.click()
        # nifty.logger.debug(" Beginning to build G Matrix")
        ...
        for A in G.matlist:
            ...
            for ival, value in enumerate(L):
                # nifty.logger.debug("val=%.4f" %value,end=' ')
                if np.abs(value) > 1e-6:
                    LargeVals += 1
                    LargeIdx.append(ival)
            # nifty.logger.debug('\n')
            # nifty.logger.debug("LargeVals %i" % LargeVals)
            tmpvecs.append(Q[:, LargeIdx])

        self.Vecs = block_matrix(tmpvecs)
        # nifty.logger.debug(" shape of DLC")
        ...
        if C is not None:
            ...
            cVecs = block_matrix.dot(
                block_matrix.dot(self.Vecs, block_matrix.transpose(self.Vecs)), Cn
            )

            # normalize C_U
            try:
                # nifty.logger.debug(cVecs.T)
    ...
    def derivatives(self, coords):
        ...
        # nifty.logger.debug(" block matrix way")
        # nifty.logger.debug(Answer)
        # tmp = block_matrix.full_matrix(self.Vecs)
        # Answer1 = np.tensordot(tmp, PrimDers, axes=(0, 0))
        # nifty.logger.debug(" np way")
        # nifty.logger.debug(Answer1)
    ...
    def second_derivatives(self, coords):
        ...
        # nifty.logger.debug(" block matrix way.")
        # nifty.logger.debug(Answer[0])

        # tmp = block_matrix.full_matrix(self.Vecs)
        # Answer2 = np.tensordot(tmp, PrimDers, axes=(0, 0))
        # nifty.logger.debug(" np tensor dot with full mat")
        # nifty.logger.debug(Answer2[0])
    ...
    def GInverse_diag(self, xyz):
        t0 = time()
        G = self.GMatrix(xyz)
        # nifty.logger.debug(G)
        dt = time() - t0
        # nifty.logger.debug(" total time to get GMatrix %.3f" % dt)
        d = np.diagonal(G)
    ...
    def create2dxyzgrid(self, xyz, xvec, yvec, nxx, nyy, mag):
        ...
        nifty.logger.debug(proj_xvec.T)
        nifty.logger.debug(proj_yvec.T)
        nifty.logger.debug(proj_xvec.shape)

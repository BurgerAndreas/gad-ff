...
class Rotator(object):
    ...
    def calc_e0(self):
        ...
        self.e0 = np.cross(
            vy, [ex, ey, ez][np.argmin([np.dot(i, ev) ** 2 for i in [ex, ey, ez]])]
        )
    ...
    def second_derivative(self, xyz, start_idx=0):
        ...
        #    return self.stored_deriv2[relative_a]
        # else:
        ##
        ...
        if self.linear:
            ...
            deriv2_raw = deriv2_raw[:-1, :, :-1, :, :]
        second_derivatives = np.zeros(
            (xyz.shape[0], 3, xyz.shape[0], 3, 3), dtype=float
        )

        # for i, a in enumerate(self.a):
        #    for j, b in enumerate(self.a):
        for i, a in enumerate(relative_a):
            for j, b in enumerate(relative_a):
                second_derivatives[a, :, b, :, :] = deriv2_raw[i, :, j, :, :]
        return second_derivatives
...
class Dihedral(PrimitiveCoordinate):
    ...
    def second_derivative(self, xyz, start_idx):
        ...
        term5 = np.outer(uxw, u + u * cu**2 - 3 * w * cu + w * cu**3) / (
            2 * lw**2 * su4
        )
        term6 = np.outer(vxw, -v - v * cv**2 + 3 * w * cv - w * cv**3) / (
            2 * lw**2 * sv4
        )
        term1 += term1.T
        term2 += term2.T
        ...
        for a in [m, n, o, p]:
            for b in [m, n, o, p]:
                deriv2[a, :, b, :] = (
                    ...
                    + (zeta(a, m, o) * zeta(b, o, p) + zeta(a, p, o) * zeta(b, o, m))
                    * term3
                    + (zeta(a, n, p) * zeta(b, p, o) + zeta(a, p, o) * zeta(b, n, p))
                    * term4

...
from utilities import units, block_matrix, manage_xyz, nifty
...
class eigenvector_follow(base_optimizer):
    ...
    def optimize(
        ...
        for ostep in range(opt_steps):
            ...
            # nifty.logger.debug(" ### Starting  line search ###")
            ls = self.Linesearch(
                nconstraints,
                x,
                fx,
            ...
            dEpre = np.dot(np.transpose(scaled_dq), gc) + 0.5 * np.dot(
                np.transpose(dEtemp), scaled_dq
            )
            dEpre *= units.KCAL_MOL_PER_AU
            # nifty.logger.debug(constraint_steps.T)

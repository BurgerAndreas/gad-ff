...
class beales_cg(base_optimizer):
    def optimize(
        ...
        #    nifty.logger.debug(" already at TS")
        #    nconstraints=1
        #    s0_prim = 0.*gp_prim
        ...
        for ostep in range(opt_steps):
            ...
            nifty.logger.debug(' stepsize = %1.2f' % stepsize)
            d = d / stepsize  # normalize
            if stepsize > self.DMAX:
                stepsize = self.DMAX
                nifty.logger.debug(' reducing step, new step = %1.2f' % stepsize)
            ...
            nifty.logger.debug(' Done linesearch')
            ...
            if ostep % xyzframerate == 0:
                ...
                manage_xyz.write_xyzs_w_comments(
                    'opt_{}.xyz'.format(molecule.node_id), geoms, energies, scale=1.0
                )

            # save variables for update Hessian!
            if (
                not molecule.coord_obj.__class__.__name__ == 'CartesianCoordinates'
                or self.options['update_hess_in_bg']
            ):
            ...
            if self.options['print_level'] > 0:
                nifty.logger.debug(
                    ' Opt step: %d E: %5.4f gradrms: %1.5f ss: %1.3f DMAX: %1.3f'
                    % (
                        ostep + 1,
                        fx - refE,
                        molecule.gradrms,
            ...
            # nifty.logger.debug(" maximum displacement component %1.2f (au)" % self.disp)
            # nifty.logger.debug(" maximum gradient component %1.2f (au)" % gmax)
            ...
            if (
                self.opt_cross
                and abs(dE) < self.conv_dE
                and molecule.gradrms < self.conv_grms
            ...
            if self.converged:
                ...
                if ostep % xyzframerate != 0:
                    ...
                    energies.append(molecule.energy - refE)
                    manage_xyz.write_xyzs_w_comments(
                        'opt_{}.xyz'.format(molecule.node_id),
                        geoms,
                        energies,
                        scale=1.0,
                    )
            ...
            if not molecule.coord_obj.__class__.__name__ == 'CartesianCoordinates':
                if opt_type == 'SEAM' or opt_type == 'MECI':
                    nifty.logger.debug(' updating DLC')
                    sys.stdout.flush()
                    # if opt_type=="ICTAN":
                    constraints = self.get_constraint_vectors(molecule, opt_type, ictan)
                    molecule.update_coordinate_basis(constraints=constraints)
                    ...
                    if nconstraints > 0:
                        g = g - np.dot(g.T, molecule.constraints) * molecule.constraints
                    g_prim = block_matrix.dot(molecule.coord_basis, g)
                    nifty.logger.debug(' Done update')
            sys.stdout.flush()

...
class GrowingString(GrowingChainOfStates):
    ...
    def __init__(self, images, calc_getter, max_cycles=75, **kwargs):
        ...
        if len(images) >= 2:
            ...
            print(
                "More than 2 images were supplied! Will only use the "
    ...
    def spline(self):
        ...
        tcks, us = zip(
            *[
                splprep(transp_coords[i : i + 9], s=0, k=3)
    ...
    def run(self):
        ...
        for self.cur_cycle in range(self.max_cycles):
            ...
            if self.images_left and ind_func(self.perp_forces[self.lf_ind]):
                ...
                # right frontier node.
                new_left_frontier = self.get_new_image(
                    self.zero_step, self.rf_ind, self.lf_ind
                )
            ...
            if self.images_left and ind_func(self.perp_forces[self.rf_ind]):
                ...
                new_right_frontier = self.get_new_image(
                    self.zero_step, self.rf_ind, self.rf_ind
                )

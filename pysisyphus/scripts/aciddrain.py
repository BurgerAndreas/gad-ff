...
class PreMinimization(Params, luigi.Task):
    ...
    def run(self):
        ...
        geom.calc = get_xtb_calc(charge=self.calc_charge(), out_dir=self.qm_out_dir)
...
class Minimization(Params, luigi.Task):
    ...
    def run(self):
        geom = geom_loader(self.input().path, coord_type="redund")
        geom.calc = get_calc(charge=self.calc_charge(), out_dir=self.qm_out_dir)
...
class SolvEnergy(Params, luigi.Task):
    ...
    def run(self):
        ...
        geom.calc = get_solv_calc(charge=self.calc_charge(), out_dir=self.qm_out_dir)
...
def run_lfer(yaml_inp):
    ...
    for name, exp_, calc in zip(names, exps, calcs):
        ...
        print(
            f"{name: >32s}: exp={exp_:{fmt}} calc={calc:{fmt}} corr={calc_corr:{fmt}}"
        )

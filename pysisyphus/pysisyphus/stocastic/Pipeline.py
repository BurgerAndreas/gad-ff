...
class Pipeline:
    ...
    def atoms_are_too_close(self, geom, factor=0.7):
    ...
    def geom_is_valid(self, geom):
        ...
        valid = geom is not None and not self.geom_is_close_in_energy(geom)

        if not self.is_analytical2d:
            valid = (
                valid and not self.atoms_are_too_close(geom) and self.geom_is_new(geom)
            )
        return valid
    ...
    def write_geoms_to_trj(self, geoms, fn, comments=None):
        with open(fn, "w") as handle:
            handle.write(make_trj_str_from_geoms(geoms, comments, energy_comments=True))
    ...
    def run(self):
        for self.cur_cycle in range(self.max_cycles):
            ...
            input_geoms = [
                self.get_input_geom(self.initial_geom) for _ in range(self.cycle_size)
            ]
            # Write input geometries to disk
            self.write_geoms_to_trj(
                input_geoms, f"cycle_{self.cur_cycle:03d}_input.trj"
            )
            ...
            for geom in opt_geoms:
                ...
                if i == 0 and len(self.new_energies) > 1:
                    ...
                    self.log(
                        f"It is a new global minimum at {energy:.4f} au! "
            ...
            if kept_geoms:
                ...
                self.log(
                    f"Kicks in cycle {self.cur_cycle} produced "
                    f"{kept_num} new geometries."
                )
                self.break_in = self.break_after
            elif self.break_in == 0:
                self.log(
                    "Didn't find any new geometries in the last "

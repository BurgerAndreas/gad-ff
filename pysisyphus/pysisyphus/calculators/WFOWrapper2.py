...
class WFOWrapper2:
    ...
    def set_data(self):
        for ci_coeffs in self.ci_coeffs_list:
            mo_inds = [self.ci_coeffs_above_thresh(state) for state in ci_coeffs]
    ...
    def get_iteration(self, ind):
        return (
            self.turbo_mos_list[ind],
            self.coords_list[ind],
            self.ci_coeffs_list[ind],
            self.mo_inds_list[ind],
            self.from_set_list[ind],
            self.to_set_list[ind],
        )
    ...
    def wf_overlap(self, ind1=-2, ind2=-1, ao_ovlp=None):
        ...
        dets1 = [gs_line] + self.make_full_dets_list(
            all_inds, det_strings, cic1_with_gs
        )
        dets2 = [gs_line] + self.make_full_dets_list(
            all_inds, det_strings, cic2_with_gs
        )
        ...
        reshaped_mats = [mat.reshape(-1, len(cic2_with_gs)) for mat in matrices]
